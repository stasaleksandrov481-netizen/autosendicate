...
- Нижняя вкладка **Меню** возвращает постоянный доступ к чатам, кланам, друзьям, кейсам, рынку, рейтингу, банку, рефералам, чип-контрактам и настройкам.
- Гараж полностью пересобран для мобильного экрана: крупная центральная машина, видимые боковые карточки, компактные статы/номер/действия и сервис под каруселью.
- Исправлен конфликт вертикального свайпа с горизонтальной каруселью гаража.
- Проверка: `npm run check:v15.1`.

## Update V16

- Исправлен mobile touch-drag карусели гаража без блокировки вертикального скролла.
- Race HUD переведён на лёгкие SVG-маркеры и отдельные полосы для 2/3/4+ участников.
- Спец-погоня ДПС получила честную инерционную модель, видимого полицейского и live gap meter.
- Исправлено обрезание showroom-карточек над нижней навигацией.
- Inline Mode теперь работает по формату `@bot @opponent`; выбор машины вызывающего идёт через native inline results, соперник выбирает свою машину кнопками в том же сообщении.
- Исправлена поддержка Telegram `inline_message_id` — это устраняет старый баг, когда после «Принять дуэль» не появлялся интерфейс выбора авто.
- Приватные Race Rooms получили live progress обоих игроков и серверно подтверждённый итог.
- Перед V16-деплоем выполнить `supabase/upgrade-v16-duel-live-progress.sql`.
- Такси/перегон/эвакуатор снижены по доходности; награды гонок и дуэлей повышены.
- Проверка: `npm run check:v16`.
