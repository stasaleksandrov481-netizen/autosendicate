...
/* ==================== STATE / STORAGE 5.0 ==================== */
const SAVE_KEY_BASE = 'autosyndicate_save_v12_7';
const GLOBAL_LEGACY_SAVE_KEYS = ['autosyndicate_save_v5','autosyndicate_save_v2'];
function authenticatedPlayerId(){
  const id=String((window as any).__AUTOSYNDICATE_SERVER_SESSION__?.playerId||'');
  return /^tg_[0-9]{1,24}$/.test(id)?id:'';
}
function playerSaveKey(){const id=authenticatedPlayerId();return id?`${SAVE_KEY_BASE}:${id}`:`${SAVE_KEY_BASE}:guest`;}
...
function defaultState(){
  return {
    ...
    heat:0, heatUpdatedAt:Date.now(), arrestUntil:0, chips:0, chipLedger:[], districtRep:0, districtWins:{},
    work:{taxiJobs:0,theftJobs:0,towTruckOwned:false,towTruckUntil:0,towTruckStartedAt:0,towTruckReward:0,activeType:'',activeUntil:0,activeStartedAt:0,activeReward:0,activeCarId:0,chipQuestDay:'',chipQuestProgress:0,chipQuestClaimed:false,deliveryReadyAt:0},
...
function normalizeCarVisuals(src){
  const out={};if(!plainObject(src))return out;
  Object.keys(src).slice(0,100).forEach((key)=>{
    if(!/^\d{1,6}$/.test(key))return;
    const carId=intNumber(key,0,1,100000);if(!carId)return;
    try{out[String(carId)]=normalizeCarVisualConfig(src[key],carId);}catch(_){out[String(carId)]=defaultCarVisualConfig(carId);}
  });
  return out;
}
...
function normalizeState(raw){
  ...
  const out={
    ...
    heat:intNumber(s.heat,0,0,5), heatUpdatedAt:intNumber(s.heatUpdatedAt,Date.now(),0,Date.now()+86400000), arrestUntil:intNumber(s.arrestUntil,0,0,Date.now()+86400000),
    chips:intNumber(s.chips,0,0,1_000_000_000), chipLedger:Array.isArray(s.chipLedger)?s.chipLedger.slice(-100):[],
    work:{
      taxiJobs:intNumber(s.work?.taxiJobs,0,0,1e9),theftJobs:intNumber(s.work?.theftJobs,0,0,1e9),
      towTruckOwned:s.work?.towTruckOwned===true,towTruckUntil:intNumber(s.work?.towTruckUntil,0,0,Date.now()+365*86400000),
      towTruckStartedAt:intNumber(s.work?.towTruckStartedAt,0,0,Date.now()+365*86400000),towTruckReward:intNumber(s.work?.towTruckReward,0,0,1e9),
      activeType:['taxi','theft','delivery'].includes(String(s.work?.activeType||''))?String(s.work.activeType):'',activeUntil:intNumber(s.work?.activeUntil,0,0,Date.now()+86400000),
      activeStartedAt:intNumber(s.work?.activeStartedAt,0,0,Date.now()+86400000),activeReward:intNumber(s.work?.activeReward,0,0,1e9),activeCarId:intNumber(s.work?.activeCarId,0,0,100000),
      chipQuestDay:safeText(s.work?.chipQuestDay,'',16),chipQuestProgress:intNumber(s.work?.chipQuestProgress,0,0,50),chipQuestClaimed:s.work?.chipQuestClaimed===true,
      deliveryReadyAt:intNumber(s.work?.deliveryReadyAt,0,0,Date.now()+365*86400000)
    },
    plates:Array.isArray(s.plates)?s.plates.slice(-100).filter(plainObject).map(p=>({country:safeText(p.country,'RU',4),region:safeText(p.region,'77',8),number:safeText(p.number,'A000AA',16),rarity:safeText(p.rarity,'common',16)})):[],
    activePlate:Number.isFinite(Number(s.activePlate))?Math.max(0,Math.floor(Number(s.activePlate))):0,
    maintenance:plainObject(s.maintenance)?s.maintenance:{},
    unreadChat:intNumber(s.unreadChat,0,0,99999),
    districtRep:intNumber(s.districtRep,0,0,1e9),districtWins:normalizeRecordNumbers(s.districtWins,0,1e9),
...
function loadState(){
  try{
    ...
    const playerId=authenticatedPlayerId();
    let raw=localStorage.getItem(SAVE_KEY);
    if(!raw){
      for(const key of LEGACY_SAVE_KEYS){
        const candidate=localStorage.getItem(key);if(!candidate||candidate.length>MAX_SAVE_BYTES)continue;
        try{
          const parsed=JSON.parse(candidate);const owner=safePlayerId(parsed?.playerId);
          // A historical shared save is only safe to migrate when it already names this exact
          // Telegram player. Unowned/shared saves are never auto-claimed by a different account.
          if(playerId && owner!==playerId)continue;
          if(!playerId && owner)continue;
          raw=candidate;break;
        }catch(_){ }
      }
    }
    if(!raw || raw.length>MAX_SAVE_BYTES){ state=defaultState();if(playerId)state.playerId=playerId;return; }
    state=normalizeState(JSON.parse(raw));
    if(playerId)state.playerId=playerId;
    localStorage.setItem(SAVE_KEY,JSON.stringify(state));
  }catch(e){ console.warn('load failed, using defaults',e); state=defaultState();const playerId=authenticatedPlayerId();if(playerId)state.playerId=playerId; }
}
...
const ECONOMY_V12_6 = {
  // v15: game-realistic ruble-like price ladder, currency name stays SYND.
  carPrices:{1:60000,2:320000,3:850000,4:1850000,5:2200000,6:3200000,7:2700000,8:2500000,9:4800000,10:3600000,11:4700000,12:4200000,13:8500000,14:11200000,15:12900000,16:19800000,17:29500000,18:16500000,19:12400000,20:28500000,21:35500000,22:52000000,23:33000000,24:48500000,25:280000000},
...
function rivalCatalogCar(opp:any){
  ...
  return list.slice().sort((a:any,b:any)=>Math.abs(Number(a.power)-power)-Math.abs(Number(b.power)-power))[0];
}
function cleanupRaceRuntimeV127(){
  const c:any=raceCtx;
  if(c){
    try{if(c.raf)cancelAnimationFrame(c.raf);}catch{}
    try{if(c.launchRaf)cancelAnimationFrame(c.launchRaf);}catch{}
    try{if(c.launchIv)clearInterval(c.launchIv);}catch{}
    try{(c.trafficTimers||[]).forEach((x:any)=>clearTimeout(x));}catch{}
    c.raf=null;c.launchRaf=null;c.launchIv=null;c.trafficTimers=[];c.gas=false;c.brake=false;
    if(c.fxCtx&&c.fxCanvas){try{c.fxCtx.clearRect(0,0,c.fxCanvas.width,c.fxCanvas.height);}catch{}}
  }
  document.body.classList.remove('race-mode');
}

const CAT_LABELS = { street:"Уличный", jdm:"JDM тюнер", muscle:"Масл-кар", sport:"Спорт", super:"Суперкар", hyper:"Гиперкар", legend:"Легенда", myth:"Редкая серия" };
...
opponentsDB.push(
  {id:'street_010',name:'Marlow',power:180,reward:160,unlockLevel:1,car:'Volkswagen Golf Mk2',rating:54,style:'Чистый старт',favoriteTracks:['Промзона'],wins:12,losses:18,avatar:'MR',taunt:'На старте не зевай.'},
  {id:'street_011',name:'Kira',power:205,reward:180,unlockLevel:1,car:'Volkswagen Golf Mk2',rating:57,style:'Точный SHIFT',favoriteTracks:['Тоннель'],wins:19,losses:16,avatar:'KR',taunt:'Одна ошибка на передаче — и меня уже нет рядом.'},
  {id:'street_012',name:'Rook',power:235,reward:205,unlockLevel:1,car:'Toyota AE86 Trueno',rating:59,style:'Резкий старт',favoriteTracks:['Промзона'],wins:24,losses:21,avatar:'RK',taunt:'Решим всё на первых ста метрах.'},
  {id:'street_013',name:'Nera',power:270,reward:240,unlockLevel:1,car:'Toyota AE86 Trueno',rating:61,style:'Ровный темп',favoriteTracks:['Эстакада'],wins:27,losses:20,avatar:'NR',taunt:'Мощность ничего не значит без темпа.'},
  {id:'street_014',name:'Hex',power:315,reward:285,unlockLevel:1,car:'Nissan Silvia S15',rating:64,style:'Давление',favoriteTracks:['Тоннель'],wins:36,losses:22,avatar:'HX',taunt:'Я буду рядом до самого финиша.'},
  {id:'street_015',name:'Astra',power:355,reward:330,unlockLevel:2,car:'Nissan Silvia S15',rating:66,style:'Высокие обороты',favoriteTracks:['Ночной проспект'],wins:41,losses:27,avatar:'AS',taunt:'Не переключайся раньше времени.'},
  {id:'street_016',name:'Vanta',power:395,reward:380,unlockLevel:2,car:'Mazda RX-7 FD',rating:69,style:'Контратака',favoriteTracks:['Порт'],wins:48,losses:25,avatar:'VT',taunt:'Выйдешь вперёд — я только ускорюсь.'},
  {id:'street_017',name:'Iris',power:430,reward:430,unlockLevel:2,car:'Toyota Supra MK4',rating:71,style:'Зацеп',favoriteTracks:['Промзона'],wins:53,losses:29,avatar:'IR',taunt:'Стартуем чисто. Без оправданий.'},
  {id:'street_018',name:'Torque',power:465,reward:485,unlockLevel:3,car:'Mitsubishi Lancer Evo IX',rating:73,style:'Момент снизу',favoriteTracks:['Порт'],wins:62,losses:31,avatar:'TQ',taunt:'Первую передачу пережить бы тебе.'},
  {id:'street_019',name:'Lynx',power:500,reward:545,unlockLevel:3,car:'Nissan Skyline GT-R R34',rating:75,style:'Ровная тяга',favoriteTracks:['Эстакада'],wins:70,losses:35,avatar:'LX',taunt:'Я быстрее, чем выгляжу.'},
  {id:'street_020',name:'Nova',power:540,reward:610,unlockLevel:3,car:'BMW M4 Competition',rating:77,style:'Launch',favoriteTracks:['Ночной проспект'],wins:78,losses:34,avatar:'NV',taunt:'Десятая секунды на старте решает больше, чем кажется.'},
  {id:'street_021',name:'Grave',power:580,reward:680,unlockLevel:4,car:'Mercedes-AMG GT',rating:79,style:'Жёсткий темп',favoriteTracks:['Тоннель'],wins:89,losses:40,avatar:'GV',taunt:'Не отпускай газ. Всё равно не поможет.'},
  {id:'street_022',name:'Vesper',power:625,reward:760,unlockLevel:4,car:'Porsche 911 Turbo S',rating:81,style:'Поздний SHIFT',favoriteTracks:['Ночной проспект'],wins:102,losses:42,avatar:'VS',taunt:'Мой мотор заканчивает фразу после твоего.'},
  {id:'street_023',name:'Dagger',power:675,reward:850,unlockLevel:5,car:'Porsche 911 Turbo S',rating:83,style:'Атака после 200',favoriteTracks:['Порт'],wins:118,losses:48,avatar:'DG',taunt:'После двухсот начинается настоящая дуэль.'},
  {id:'street_024',name:'Sable',power:725,reward:950,unlockLevel:5,car:'Audi R8 V10',rating:85,style:'Холодный темп',favoriteTracks:['Эстакада'],wins:133,losses:44,avatar:'SB',taunt:'Ровный темп всегда побеждает панику.'},
  {id:'street_025',name:'Knox',power:775,reward:1080,unlockLevel:6,car:'Nissan GT-R R35',rating:87,style:'Максимальное давление',favoriteTracks:['Тоннель'],wins:151,losses:50,avatar:'KX',taunt:'Оставь полметра — я заберу всю полосу.'},
  {id:'street_026',name:'Cinder',power:825,reward:1220,unlockLevel:6,car:'McLaren 720S',rating:88,style:'Поздний рывок',favoriteTracks:['Порт'],wins:166,losses:53,avatar:'CI',taunt:'Не радуйся лидерству раньше финиша.'},
  {id:'street_027',name:'Onyx',power:875,reward:1380,unlockLevel:7,car:'Ferrari 488 Pista',rating:89,style:'Чистые зоны',favoriteTracks:['Ночной проспект'],wins:184,losses:55,avatar:'OX',taunt:'Зелёная зона или поражение.'},
  {id:'street_028',name:'Specter',power:925,reward:1560,unlockLevel:7,car:'Ferrari SF90 Stradale',rating:91,style:'Безошибочный',favoriteTracks:['Промзона'],wins:205,losses:57,avatar:'SP',taunt:'Я считаю твои ошибки ещё до старта.'},
  {id:'street_029',name:'Valkyr',power:975,reward:1780,unlockLevel:8,car:'Lamborghini Aventador',rating:92,style:'Агрессивный',favoriteTracks:['Эстакада'],wins:229,losses:61,avatar:'VK',taunt:'Финиш увидит только один лидер.'},
  {id:'street_030',name:'Nocturne',power:1030,reward:2040,unlockLevel:8,car:'Lamborghini Aventador',rating:93,style:'Ночной темп',favoriteTracks:['Ночной проспект'],wins:254,losses:62,avatar:'NT',taunt:'Ночью ошибки видно лучше.'},
  {id:'street_031',name:'Blackstar',power:1090,reward:2350,unlockLevel:9,car:'Bugatti Chiron',rating:94,style:'Тяга сверху',favoriteTracks:['Порт'],wins:281,losses:64,avatar:'BS',taunt:'На шестой передаче поговорим.'},
  {id:'street_032',name:'Monarch',power:1160,reward:2720,unlockLevel:10,car:'Bugatti Chiron',rating:96,style:'Давление',favoriteTracks:['Тоннель'],wins:317,losses:59,avatar:'MN',taunt:'Эта сетка заканчивается мной.',boss:true},
  {id:'street_033',name:'Warden',power:1240,reward:3200,unlockLevel:11,car:'Bugatti Chiron',rating:97,style:'Контроль',favoriteTracks:['Промзона'],wins:352,losses:55,avatar:'WD',taunt:'Дорога узкая. Ошибка одна.',boss:true},
  {id:'street_034',name:'Requiem',power:1320,reward:3800,unlockLevel:13,car:'Bugatti Chiron',rating:98,style:'Финишер',favoriteTracks:['Порт'],wins:401,losses:49,avatar:'RQ',taunt:'Не проиграй гонку до зелёного.',boss:true},
  {id:'street_035',name:'Carbon Ghost',power:1410,reward:4600,unlockLevel:15,car:'Bugatti Chiron',rating:99,style:'Абсолютный темп',favoriteTracks:['Ночной проспект'],wins:477,losses:41,avatar:'CG',taunt:'Поймай меня, если сможешь.',boss:true},
  {id:'street_036',name:'Apex',power:1500,reward:5800,unlockLevel:18,car:'Bugatti Chiron',rating:100,style:'Финальный босс',favoriteTracks:['Карбоновое шоссе'],wins:601,losses:27,avatar:'AP',taunt:'Последняя линия. Докажи, что дошёл сюда не случайно.',boss:true}
);
const tournamentsDB = [
...
function entryFeeFor(opp){ return opp.entryFee!==undefined ? opp.entryFee : Math.max(2500,Math.round((Number(opp.reward)||0)*0.10)); }
function fuelCostFor(opp){ return opp.boss ? 26 : (opp.entryFee!==undefined ? 22 : 14); }

/* ==================== JOBS ==================== */
const jobsDB = [
  { id:"wash", name:"Мойка тачек в гараже", desc:"Стабильный небольшой заработок", reward:8000, xp:4, cooldown:90 },
  { id:"delivery", name:"Доставка запчастей", desc:"Средняя смена с нормальной выплатой", reward:14000, xp:7, cooldown:240 },
  { id:"taxi", name:"Ночной таксист", desc:"Длинная смена с повышенной выплатой", reward:18000, xp:11, cooldown:600 }
];
...
const POLICE_BASE_FINE = 12000;
const LICENSE_BASE_PRICE = 45000;
function licensePrice(){ return LICENSE_BASE_PRICE + state.licenseSuspendCount*15000; }

/* ==================== БАНК ==================== */
const BANK_MAX_PER_TRANSFER = 250000;
const BANK_MAX_PER_DAY = 750000;
...
function addXP(n){
  ...
  if(leveled){ const bonus=Math.min(75000,5000+state.level*1500); awardMoney(bonus,'ПОВЫШЕНИЕ УРОВНЯ'); showToast('Новый уровень · LVL '+state.level); }
  updateHeader();
}
...
function tapLogo(){
  ...
  state.secretBonusAt=now; state.coins+=15000; state.stats.totalEarned+=15000; state.nitro+=1;
  showToast(' Тайник синдиката: +15 000 SYND и 1 нитро');
...
function renderJobs(){
  ...
  jobsDB.forEach(job=>{
    ...
    container.innerHTML += '<div class="job-card">'+
        '<div class="job-head"><span class="job-name">'+job.name+'</span><span class="job-reward">+'+fmt(job.reward)+' SYND</span></div>'+
...
function doJob(jobId){
  ...
  state.coins=(Number(state.coins)||0)+job.reward;state.stats.totalEarned=(Number(state.stats.totalEarned)||0)+job.reward;addXP(job.xp||5);state.jobCooldowns[jobId]=now+job.cooldown*1000;
...
setInterval(()=>{
  ...
  if(document.getElementById('screen-chips')?.classList.contains('active')) (window as any).renderChipCenterV15?.();
},1000);
...
const achievementsDB = [
  { id:'first_win', name:'Первая кровь', desc:'Выиграй свою первую дуэль', icon:'', reward:12000, check:s=>s.stats.wins>=1 },
  { id:'ten_wins', name:'Ветеран трассы', desc:'Одержи 10 побед', icon:'', reward:65000, check:s=>s.stats.wins>=10 },
  { id:'five_cars', name:'Коллекционер', desc:'Владей 5 машинами одновременно', icon:'', reward:90000, check:s=>s.ownedCars.length>=5 },
  { id:'all_cars', name:'Весь гараж синдиката', desc:'Собери все машины в игре', icon:'', reward:750000, check:s=>s.ownedCars.length>=25 },
  { id:'lvl10', name:'Авторитет района', desc:'Достигни 10 уровня', icon:'⭐', reward:120000, check:s=>s.level>=10 },
  { id:'first_fine', name:'Знакомство с ДПС', desc:'Получи первый штраф от полиции', icon:'', reward:8000, check:s=>s.stats.finesCount>=1 },
  { id:'bj_win', name:'Карточный игрок', desc:'Выиграй раунд в блэкджек', icon:'', reward:18000, check:s=>(s.stats.blackjackWins||0)>=1 },
  { id:'max_tune', name:'Гараж мечты', desc:'Прокачай тюнинг любой машины до максимума во всех категориях', icon:'', reward:250000, check:s=>Object.values(s.upgrades).some(u=>u && TUNE_TYPES.every(t=>u[t.key]>=t.hpPerStage.length)) },
  { id:'earn50k', name:'Барон подполья', desc:'Заработай суммарно 50 000 ', icon:'', reward:150000, check:s=>s.stats.totalEarned>=50000 },
  { id:'daily7', name:'Верный синдикату', desc:'Забирай ежедневную награду 7 дней подряд', icon:'', reward:100000, check:s=>s.dailyStreak>=7 },
  { id:'secret_car', name:'Тень подполья', desc:'Стань владельцем легендарной машины', icon:'', reward:500000, check:s=>s.ownedCars.includes(24)||s.ownedCars.includes(25) },
  { id:'boss_slayer', name:'Убийца боссов', desc:'Победи одного из боссов подполья', icon:'', reward:250000, check:s=>(s.stats.bossWins||0)>=1 }
];
...
const casesDB = [
  ...
  { id:'gold', name:'Золотой кейс', icon:'', price:4000, desc:'Крупный куш, нитро и редкие награды' }
];
...
function openCase(caseId){
  ...
  if(caseId==='bronze'){
    ...
    if(r<0.45){ const c=Math.round(cs.price*(0.6+Math.random())); state.coins+=c; resultMsg='+'+fmt(c)+' '; }
      ...
      const available=realCarCatalog().filter(c=>c.id>=18&&c.id<=25&&!state.ownedCars.includes(c.id));
      if(available.length>0){
        const car=available[Math.floor(Math.random()*available.length)];
        state.ownedCars.push(car.id);
        resultMsg='ЛЕГЕНДАРНЫЙ ДРОП! '+car.name+' теперь в гараже.';
      } else { const c=Math.round(cs.price*5); state.coins+=c; resultMsg='Джекпот! +'+fmt(c)+' SYND'; }
    }
  }
  showToast(' '+resultMsg);
...
async function renderLeaderboard(){
  ...
  rows.forEach((r,i)=>{
    ...
    row.innerHTML='<div class="lb-rank '+rankCls+'">#'+(i+1)+'</div>'+
      '<div class="lb-name">'+escapeHtml(r.name||'Гонщик')+(me?' <small style="color:var(--green)">ВЫ</small>':'')+tagsRowHTML(r.profile_tags,true)+
...
const CONTRACT_POOL=[
  {id:'races3',event:'race',target:3,reward:12000,name:'На линии',desc:'Заверши 3 уличных заезда.'},
  {id:'wins2',event:'win',target:2,reward:18000,name:'Без права на ошибку',desc:'Выиграй 2 заезда.'},
  {id:'shift4',event:'perfectShift',target:4,reward:15000,name:'Идеальная КПП',desc:'Сделай 4 идеальных переключения.'},
  {id:'job2',event:'job',target:2,reward:10000,name:'Запасной план',desc:'Выполни 2 подработки.'},
  {id:'nitro1',event:'nitro',target:1,reward:12000,name:'На полном баллоне',desc:'Используй нитро в заезде.'},
  {id:'buy1',event:'buy',target:1,reward:25000,name:'Расширение гаража',desc:'Купи одну машину.'}
];
...
/* ==================== DAILY REWARD ==================== */
const DAILY_REWARDS = [8000,12000,18000,26000,38000,55000,85000];
...
function prepareRace(target,mode){
  ...
  const maxSpeed=Math.round(Math.max(170,Math.min(430,160+rawPower*.20)));
  const aiMaxSpeed=Math.round(Math.max(165,Math.min(430,160+(Number(opp.power)||rawPower)*.20)));
...
function showRaceCockpit(){
  ...
  '<div class="analog-gauge speed3"><div class="gauge-caption">СКОРОСТЬ</div><div class="dial speed-dial zone-dial" id="speed-dial"><div class="dial-ticks"></div><div class="dial-needle speed-needle" id="speed-needle"></div><div class="dial-hub"></div><div class="gauge-center"><b id="race-speed">0</b><span>КМ/Ч</span></div></div><div class="gauge-scale"><span>0</span><span>'+c.maxSpeed+'</span></div></div></div>'+
  '<div class="race-controls"><button class="race-control pedal brake" id="brake-btn" onpointerdown="raceHold(\'brake\',true)" onpointerup="raceHold(\'brake\',false)" onpointercancel="raceHold(\'brake\',false)" onpointerleave="raceHold(\'brake\',false)"><span class="pedal-face">ТОРМ.</span><small>ТОРМОЗ</small></button><button class="race-control pedal gas" id="gas-btn" onpointerdown="raceHold(\'gas\',true)" onpointerup="raceHold(\'gas\',false)" onpointercancel="raceHold(\'gas\',false)" onpointerleave="raceHold(\'gas\',false)"><span class="pedal-face">ГАЗ</span><small>ПЕДАЛЬ</small></button><button class="race-control shift" id="shift-btn" onclick="manualShift()"><span class="shift-face">↑</span><small id="shift-label">ПЕРЕДАЧА · 1→2</small></button><button class="race-control nitro" id="nitro-btn" onclick="useRaceNitro()"><span class="nitro-face">N₂O</span><small id="nitro-label">'+state.nitro+' ЗАРЯД.</small><div class="race-nitro-bar"><i id="nitro-bar-fill"></i></div></button></div>'+
...
function startTrafficLight(){
  ...
  steps[0]();
  c.trafficTimers=[setTimeout(steps[1],650),setTimeout(steps[2],1250),setTimeout(steps[3],1850)];
}
...
function raceFrame(now){
  ...
  if(c.uiTimer>=.08){c.uiTimer=0;updateRaceHUD();}
...
function updateRaceHUD(){
  ...
  if(gapEl){
    ...
    else if(gap>0) gapEl.innerText='ТЫ +'+Math.abs(gap).toFixed(1)+' м ВПЕРЕДИ';
    else gapEl.innerText='СОПЕРНИК +'+Math.abs(gap).toFixed(1)+' м ВПЕРЕДИ';
  }
  if(leadEl){
...
function bindLocalStateToVerifiedPlayer(session:any){
  const pid=String(session?.playerId||'');if(!/^tg_[0-9]{1,24}$/.test(pid))return;
  const target=`${SAVE_KEY_BASE}:${pid}`;
  if(SAVE_KEY===target){state.playerId=pid;return;}
  const previousState=state,previousKey=SAVE_KEY;
  SAVE_KEY=target;
  const stored=localStorage.getItem(target);
  if(stored&&stored.length<=MAX_SAVE_BYTES){
    try{state=normalizeState(JSON.parse(stored));}catch(_){state=defaultState();}
  }else{
    const previousOwner=safePlayerId(previousState?.playerId);
    state=(!previousOwner||previousOwner===pid)?normalizeState(previousState):defaultState();
  }
  state.playerId=pid;
  if(session?.name)state.playerName=safeText(session.name,'Гонщик',48);
  if(session?.username)state.playerUsername=safeText(session.username,'',32).replace(/^@/,'').replace(/[^A-Za-z0-9_]/g,'');
  try{localStorage.setItem(target,JSON.stringify(state));}catch(_){ }
  if(previousKey!==target)console.info('AutoSyndicate player storage bound',pid);
}
...
async function ensureOnlineAuth(){
  try{
    if(window.__AUTOSYNDICATE_AUTHENTICATED__===true&&window.__AUTOSYNDICATE_SERVER_SESSION__?.playerId){
      bindLocalStateToVerifiedPlayer(window.__AUTOSYNDICATE_SERVER_SESSION__);onlineAuthReady=true;
    }else{
      ...
      if(response.ok&&payload?.authenticated&&payload?.session?.playerId){
        ...
        bindLocalStateToVerifiedPlayer(payload.session);
...
async function bootstrapOnline(){
  ...
  if(!onlineIntervalsStarted){onlineIntervalsStarted=true;setInterval(()=>{if(document.visibilityState==='visible')pollBackgroundClaims();},120000);setInterval(()=>{if(document.visibilityState==='visible'&&onlineAuthReady){syncPlayerProfile();checkServerSync();}},120000);}
}
...
async function recoverServerSession(){
  try{
    ...
    if(onlineAuthReady){
      ...
      bindLocalStateToVerifiedPlayer(s);
    }
    return onlineAuthReady;
  }catch(e){console.warn('Telegram session recovery failed',e);return false;}
}
const serverBackoffByRoute=new Map<string,number>();
async function serverFetch(input,init={}){
  const path=String(input||'');
  const route=(()=>{try{return new URL(path,location.origin).pathname;}catch{return path;}})();
  const bypassBackoff=/\/api\/(auth\/telegram|session)/.test(route);
  const backoff=serverBackoffByRoute.get(route)||0;
  if(!bypassBackoff&&Date.now()<backoff){
    return new Response(JSON.stringify({ok:false,error:'online service is retrying',code:'ROUTE_BACKOFF'}),{status:503,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}});
  }
  const run=async()=>{
    ...
    try{
      ...
      if(response.status===503)serverBackoffByRoute.set(route,Date.now()+3500);else if(response.ok)serverBackoffByRoute.delete(route);
      return response;
    }catch(e){serverReachable=false;serverBackoffByRoute.set(route,Date.now()+3500);throw e;}
  ...
  if(!response.ok&&response.status!==401&&response.status!==503){
    try{const detail=await response.clone().json();console.warn('AutoSyndicate API request failed',{path:route,status:response.status,code:detail?.code||'REQUEST_FAILED'});}catch(_){ }
  }
  return response;
}
...
function playerProfilePayload(){
  return {
    ...
    profile_tags: normalizeTags(state.profileTags)
  };
}
...
(function(){
  const V8_VERSION=8;
  const BACKUP_KEYS=playerBackupKeys();
  ...
  saveState=function(){
    try{
      ...
      SAVE_KEY=playerSaveKey();const backups=playerBackupKeys();
      const current=localStorage.getItem(SAVE_KEY);
      if(current && current.length<=MAX_SAVE_BYTES){
        const prev1=localStorage.getItem(backups[0]),prev2=localStorage.getItem(backups[1]);
        if(prev2)localStorage.setItem(backups[2],prev2);
        if(prev1)localStorage.setItem(backups[1],prev1);
        localStorage.setItem(backups[0],current);
      }
    }catch(_){ }
    baseSaveState();
  };
  loadState=function(){
    SAVE_KEY=playerSaveKey();
    const backups=playerBackupKeys(),playerId=authenticatedPlayerId();
    const candidates=[SAVE_KEY,...backups,...LEGACY_SAVE_KEYS];
    for(const key of candidates){
      try{
        const raw=localStorage.getItem(key);if(!raw||raw.length>MAX_SAVE_BYTES)continue;
        const parsed=JSON.parse(raw),owner=safePlayerId(parsed?.playerId);
        if(playerId&&key!==SAVE_KEY&&owner&&owner!==playerId)continue;
        state=normalizeState(parsed);if(playerId)state.playerId=playerId;localStorage.setItem(SAVE_KEY,JSON.stringify(state));return;
      }catch(_){ }
    }
    state=defaultState();if(playerId)state.playerId=playerId;
  };
  ...
  const EXTRA_RIVALS=[
    ...
    {id:'npc_061',name:'Ghostline',power:1700,reward:18500,unlockLevel:20,car:'Bugatti Chiron',rating:99,style:'Без следа',favoriteTracks:['Тоннель','Ночной проспект'],wins:536,losses:27,avatar:'GL',taunt:'Увидишь только мои фонари.',preLines:['Не потеряй линию.'],winLine:'След исчез.',loseLine:'Ты остался рядом.',boss:true},
    {id:'npc_062',name:'Sovereign',power:1800,reward:22500,unlockLevel:22,car:'Lamborghini Aventador',rating:100,style:'Контроль дистанции',favoriteTracks:['Промзона','Портовый обход'],wins:604,losses:24,avatar:'SV',taunt:'Я не выигрываю метрами. Я забираю дистанцию.',preLines:['Считай каждый метр.'],winLine:'Дистанция принадлежит мне.',loseLine:'Ты отнял её.',boss:true},
  ...
  function makeCasePrize(cs,forcedRarity){
    ...
    if(cs.id==='gold'&&((rarity==='legendary'&&typeRoll>.88)||(rarity==='mythic'&&typeRoll>.70)||typeRoll>.9965)){
      const reserved=escrowCarIds();const possible=carsDB.filter(c=>c.image&&c.id>=18&&c.id<=25&&!state.ownedCars.includes(c.id)&&!reserved.has(c.id));if(possible.length){const car=pick(possible);return {type:'car',rarity:rarity==='mythic'?'mythic':'legendary',label:car.name,carId:car.id};}
    }
  ...
  const basePoll=pollBackgroundClaims;pollBackgroundClaims=function(){basePoll();};
...
(function(){
  'use strict';
  const V9_VERSION=9;
  const GEAR_CAPS=[0,42,94,158,225,300,430];
  ...
  prepareRace=function(target,mode){
    ...
    const c=raceCtx;if(!c)return;document.body.classList.add('race-mode');
  ...
  showRaceCockpit=function(){
    ...
    if(map){
      ...
      const me=document.getElementById('map-me'),ai=document.getElementById('map-ai'),myCar=activeCar(),enemyCar=rivalCatalogCar(c.opp);
      if(me)me.innerHTML='<i class="race-car-shape player has-image" style="background-image:url('+escapeHtml(carThumb(myCar))+')"><b></b></i>';
      if(ai)ai.innerHTML='<i class="race-car-shape rival has-image" style="background-image:url('+escapeHtml(carThumb(enemyCar))+')"><b></b></i>';
    }
    ...
    const help=document.getElementById('shift-help');
    if(help)help.innerHTML='1-я: 42 · 2-я: 94 · 3-я: 158 · 4-я: 225 · 5-я: 300 · 6-я: до 430 км/ч';
    updateRaceZones();updateRaceHUD();
  };

  function initRaceFx(c){
    const canvas=c.fxCanvas;if(!canvas)return;
    const lowEnd=!!c.fxLowQuality||(navigator.hardwareConcurrency&&navigator.hardwareConcurrency<=6)||(navigator.deviceMemory&&navigator.deviceMemory<=6);
    ...
    const count=lowEnd?0:6;
    c.fxParticles=Array.from({length:count},(_,i)=>({x:(i+.5)/count,y:rand(),len:.08+rand()*.16,speed:.65+rand()*.8}));
  }
  function renderRaceFx(c,dt){
    if(!c?.fxCtx||!c.fxCanvas||state.settings?.reducedMotion||c.fxLowQuality)return;
    c.fxFrameTick=(c.fxFrameTick||0)+1;if(c.fxFrameTick%2)return;
  ...
  function simulateAiV9(c,dt){
    ...
    const hpFactor=clamp((Math.max(120,Number(c.opp.power)||300)/300)*.90,.50,2.65);
  ...
  simulateRace=function(dt){
    ...
    if(c.gas&&!c.brake){
      ...
      const hpFactor=clamp(hp/300,.55,2.85),limiter=clamp((gearCap-c.speed)/12,.025,1),nitro=c.nitroActive?1.19:1,boost=c.shiftBoost||1;
      ...
      const aero=Math.max(0,(c.speed/430)*(c.speed/430))*3.2;
    ...
    c.distance=Math.min(c.trackLength,c.distance+(c.speed/3.6)*dt);
    if(c.chase||c.isPolice) simulateChaseAiV14(c,dt); else simulateAiV9(c,dt);
  ...
  raceFrame=function(now){
    ...
    c.frameEma=c.frameEma?c.frameEma*.94+dt*.06:dt;if(c.frameEma>.022&&!c.fxLowQuality){c.fxLowQuality=true;initRaceFx(c);const r=document.querySelector('#race-content .race3');if(r)r.classList.add('race-low-fx');}
  ...
  (window as any).reconcileCaseRollsV15=reconcileCaseRolls;
  ...
  window.createClanV9=async function(){const n=document.getElementById('clan-name-v9')?.value||'',isOpen=!!document.getElementById('clan-mode-open-v14')?.checked;try{await socialApi('/api/social/clans','POST',{action:'create',name:n,isOpen},'Кланы');showToast('Клан создан');await loadClanV9();}catch(e){showToast(safeText(e?.message||'Не удалось создать клан','Ошибка',110));}};
  window.setClanVisibilityV14=async function(isOpen){try{await socialApi('/api/social/clans','POST',{action:'visibility',isOpen:!!isOpen},'Кланы');showToast(isOpen?'Клан открыт для всех':'Клан только по приглашению');await loadClanV9();}catch(e){showToast(safeText(e?.message||'Не удалось изменить режим','Ошибка',110));}};
  window.joinOpenClanV14=async function(clanId){try{await socialApi('/api/social/clans','POST',{action:'joinOpen',clanId},'Кланы');showToast('Ты вступил в клан');await loadClanV9();}catch(e){showToast(safeText(e?.message||'Не удалось вступить','Ошибка',110));}};
  ...
  async function loadClanV9(){
    ...
    try{
      const payload=await socialApi('/api/social/clans','GET',null,'Кланы'),membership=payload?.membership||null,invites=Array.isArray(payload?.invites)?payload.invites:[],members=Array.isArray(payload?.members)?payload.members:[],lb=payload?.clanRank||null,openClans=Array.isArray(payload?.openClans)?payload.openClans:[];const openMap=new Map(openClans.map(x=>[String(x.id),true]));clanLeaderboardCache=(Array.isArray(payload?.leaderboard)?payload.leaderboard:[]).map(x=>({...x,is_open:openMap.has(String(x.id))}));
      if(!membership){root.innerHTML='<div class="clan-create-v9"><b>СОЗДАТЬ КЛАН</b><span>Выбери режим вступления.</span><input id="clan-name-v9" maxlength="24" placeholder="Название клана"><div class="clan-mode-v14"><label><input type="radio" name="clan-mode-v14" value="closed" checked> 🔒 По приглашению</label><label><input id="clan-mode-open-v14" type="radio" name="clan-mode-v14" value="open"> 🔓 Открытый</label></div><button class="btn btn-select" onclick="createClanV9()">СОЗДАТЬ</button></div>'+renderOpenClansV14(openClans)+renderClanInvites(invites);currentClanDivision='Мантика';loadClanLeaderboardV9();return;}
      const clan=relationOne(membership.clans);currentClanDivision=lb?.division||'Мантика';const isOwner=membership.role==='owner';
      root.innerHTML='<div class="clan-hero-v9"><div><span>КЛАН · '+(clan.is_open?'🔓 ОТКРЫТЫЙ':'🔒 ПО ПРИГЛАШЕНИЮ')+'</span><b>'+escapeHtml(clan.name||'Клан')+'</b><small>'+escapeHtml(currentClanDivision)+' · '+fmt(lb?.score||0)+' очк. · #'+(lb?.global_rank||'—')+'</small></div><button class="btn btn-ghost" onclick="leaveClanV9()">ВЫЙТИ</button></div>'+(isOwner?'<div class="clan-mode-v14 settings"><button class="'+(!clan.is_open?'active':'')+'" onclick="setClanVisibilityV14(false)">🔒 ПО ПРИГЛАШЕНИЮ</button><button class="'+(clan.is_open?'active':'')+'" onclick="setClanVisibilityV14(true)">🔓 ОТКРЫТЫЙ</button></div><div class="social-search-v9"><input id="clan-invite-v9" maxlength="90" placeholder="ID или @login друга"><button class="btn btn-select" onclick="inviteClanV9()">ПРИГЛАСИТЬ</button></div>':'')+'<div class="v9-section-head"><b>СОСТАВ</b><span>'+members.length+'</span></div>'+members.map(m=>{const profile=relationOne(m.player_profiles);return '<div class="clan-member-v9"><div><b>'+escapeHtml(m.player_name)+' '+tagsRowHTML(profile?.profile_tags,true)+'</b><span>'+escapeHtml(({owner:'ВЛАДЕЛЕЦ',officer:'ОФИЦЕР',member:'УЧАСТНИК'}[String(m.role||'member')]||'УЧАСТНИК'))+' · РЕЙТИНГ '+(profile?.rating||0)+' · '+escapeHtml(profile?.current_car_name||'машина не указана')+(Number(profile?.wanted_level)>=5?' · 🚨 5★ РОЗЫСК':'')+'</span></div>'+(isOwner&&m.role!=='owner'?'<button class="btn btn-ghost" onclick="kickClanMemberV9('+jsArg(m.member_uid)+')">ИСКЛЮЧИТЬ</button>':'')+'</div>';}).join('')+renderClanInvites(invites);loadClanLeaderboardV9();
    }catch(e){console.warn('clan unavailable',e);root.innerHTML='<div class="empty-note">Клановый раздел сейчас недоступен.</div>';}
  }
  function renderOpenClansV14(rows){if(!rows.length)return'<div class="empty-note compact-v9">Открытых кланов пока нет.</div>';return '<div class="v9-section-head"><b>ОТКРЫТЫЕ КЛАНЫ</b><span>'+rows.length+'</span></div>'+rows.map(c=>'<div class="social-row-v9"><div><b>🔓 '+escapeHtml(c.name||'Клан')+'</b><span>Можно вступить сразу</span></div><button class="btn btn-select" onclick="joinOpenClanV14('+jsArg(c.id)+')">ВСТУПИТЬ</button></div>').join('');}
  ...
  switchTab=function(tabId){ensureV9Screens();if(tabId!=='chat'&&chatPollTimer){clearInterval(chatPollTimer);chatPollTimer=null;}if(tabId!=='race')cleanupRaceRuntimeV127();v8SwitchTab(tabId);if(tabId==='slots')initSlotsV9();if(tabId==='friends')loadFriendsV9();if(tabId==='clans')loadClanV9();if(tabId==='cases'){reconcileCaseRolls();}};
...
(() => {
  ...
  const serverPlayer=bootstrap.player;
  if(serverPlayer && String(serverPlayer.id||'')===authenticatedPlayerId()){
    const serverOwned=Array.isArray(serverPlayer.owned_cars)?serverPlayer.owned_cars.map(Number).filter((id:any)=>Number.isInteger(id)&&id>=1&&id<=25):[];
    // Server state is authoritative for identity and starter ownership. Existing local career
    // progress is kept until every economy mutation has moved server-side; clean/new clients
    // receive the same starter state on every device.
    const localIsFresh=(Number(state.stats?.races)||0)===0 && (Number(state.stats?.totalEarned)||0)===0 && (state.ownedCars||[]).length<=1;
    if(localIsFresh){
      state.coins=Math.max(0,Number(serverPlayer.balance ?? 1500));
      state.level=Math.max(1,Number(serverPlayer.level)||1);
      state.xp=Math.max(0,Number(serverPlayer.xp)||0);
      state.ownedCars=serverOwned.length?serverOwned:[1];
      state.activeCarId=state.ownedCars.includes(Number(serverPlayer.active_car_id))?Number(serverPlayer.active_car_id):state.ownedCars[0];
      state.stats.races=Math.max(0,Number(serverPlayer.races)||0);
  ...
  function duelPoolV127(myPower:number){
    const source=opponentsDB.filter((o:any)=>o && !String(o.id).startsWith('tgduel_'));
    let pool:any[];
    if(duelFilter==='boss') pool=source.filter((o:any)=>o.boss);
    else {
      // Normal street rivals are matched by build, not hidden behind level gates. Bosses keep progression.
      pool=source.filter((o:any)=>!o.boss || state.level>=Number(o.unlockLevel||1));
      if(duelFilter==='equal') pool=pool.filter((o:any)=>Math.abs(Number(o.power)-myPower)/Math.max(myPower,1)<=.30 && !o.boss);
      if(duelFilter==='risk') pool=pool.filter((o:any)=>Number(o.power)>myPower*1.08 && Number(o.power)<=myPower*2.35 && !o.boss);
    }
    // Never render an empty network because a narrow filter happened to have no exact hit.
    if(!pool.length && duelFilter!=='boss') pool=source.filter((o:any)=>!o.boss);
    return pool.slice().sort((a:any,b:any)=>{
      if(duelFilter==='boss') return Number(a.unlockLevel||1)-Number(b.unlockLevel||1)||Number(a.power)-Number(b.power);
      if(duelFilter==='risk') return Number(a.power)-Number(b.power);
  ...
  renderOpponents=function(){
    ...
    root.innerHTML='';
    const car=carsDB.find((x:any)=>x.id===state.activeCarId) || realCarCatalog()[0];
    if(!car){root.innerHTML='<div class="empty-note">В гараже пока нет доступной машины.</div>';return;}
    if(state.licenseSuspended){root.innerHTML='<div class="empty-note">Доступ к уличным заездам временно приостановлен.</div>';return;}
    const myPower=getEffectivePower(car),history=state.raceHistory||[];
    let pool:any[];
    if(state.duelSub==='tour'){
      const now=Date.now(),day=new Date().toISOString().slice(0,10);
      pool=tournamentsDB.filter((o:any)=>state.level>=Number(o.unlockLevel||1)).filter((o:any)=>{const r=state.tournamentRuns[String(o.id)]||{};const count=r.day===day?(Number(r.count)||0):0,next=r.day===day?(Number(r.next)||0):0;return count<3&&next<=now;});
    }else pool=duelPoolV127(myPower);
    ...
    if(summary){summary.innerHTML='<div class="summary-main"><div class="summary-car-img"><img src="'+escapeHtml(carThumb(car))+'" alt=""></div><div><b>'+escapeHtml(car.name)+'</b><span>'+fmt(myPower)+' л.с. · '+pool.length+' соперников</span></div></div>'+(state.duelSub==='tour'?'':'<div class="duel-pager"><button '+(duelPage<=0?'disabled':'')+' onclick="setDuelPage(-1)">‹</button><b>'+(duelPage+1)+' / '+totalPages+'</b><button '+(duelPage>=totalPages-1?'disabled':'')+' onclick="setDuelPage(1)">›</button></div>');}
...
/* ==================== CARBON FEATURE PACK 13 ====================
   Dynamic rivals, CHIP economy, street chaos, police chase, hardcore
   shifting, country plates, garage/dealer carousel and maintenance. */
(function carbonFeaturePack(){
  const BOT_NAMES=[
    'Шахист_77','StreetKing','Дядя Вася','TurboRacer','Smokie_Rider','NightFox',
    'DriftMaster','Black_ICE','Механик_13','Redline','GhostDriver','Кот_На_Газу',
    'V8_Baron','LowKey','CarbonWolf','Racer_404','Дрифтер_88','TurboVanya'
  ];
  const BOT_STATUS=[
    'Ищет заезд','На линии','Готов к старту','Охота за рейтингом','Не тормозит',
    'В гараже','Ловит зелёную зону','В розыске','Разогрет','Ждёт вызова'
  ];
  const BOSS_TITLES=['Император асфальта','Король Карбона','Владыка Ночной трассы','Лорд Синдиката','Призрачный чемпион'];
  const BOSS_PREFIX=['Ω','X','R','V','Σ','ZX'];
  const COUNTRIES={
    RU:{name:'Россия',flag:'🇷🇺',regions:['77','99','97','50','78','116','174','196']},
    UA:{name:'Украина',flag:'🇺🇦',regions:['AA','AB','AE','AH','AI','AX','BK']},
    KZ:{name:'Казахстан',flag:'🇰🇿',regions:['01','02','05','07','08','10','17']}
  };
  const PLATE_LETTERS=['A','B','C','E','H','K','M','O','P','T','X','Y'];
  const CARBON_FEATURES={
    normal:{label:'1V1 · ДУЭЛЬ',lanes:2},
    chaos:{label:'УЛИЧНЫЙ ХАОС · 4 ЛИНИИ',lanes:4},
    chase:{label:'СПЕЦ-ПОГОНЯ · ДПС',lanes:3},
    hardcore:{label:'ИДЕАЛЬНОЕ ПЕРЕКЛЮЧЕНИЕ · HARDCORE',lanes:2}
  };
  function rand(a){return a[Math.floor(Math.random()*a.length)]}
  function dynName(opp, idx=0, boss=false){
    if(boss){
      const title=rand(BOSS_TITLES), prefix=rand(BOSS_PREFIX);
      return prefix+' '+title+' · '+String(opp?.name||'Босс').replace(/^RIVAL$/i,'');
    }
    return rand(BOT_NAMES)+' · '+rand(BOT_STATUS);
  }
  function bossify(opp){return {...opp,name:dynName(opp,0,true),status:'Уникальный босс'}}

  function ensureFeatureState(){
    state.chips=Number.isFinite(Number(state.chips))?Math.max(0,Number(state.chips)):0;
    if(!Array.isArray(state.chipLedger))state.chipLedger=[];
    if(!Array.isArray(state.plates))state.plates=[];
    if(!plainObject(state.maintenance))state.maintenance={};
    if(!Number.isFinite(Number(state.unreadChat)))state.unreadChat=0;
  }
  ensureFeatureState();
  ...
  function updateHeaderFeature(){
    updateHeader();
    const h=document.getElementById('chips-display'); if(h)h.textContent=fmt(state.chips);
    const c=document.getElementById('coins-display'); if(c)c.textContent=fmt(state.coins);
    const badge=document.getElementById('chat-unread-badge');if(badge)badge.textContent=state.unreadChat>99?'99+':String(state.unreadChat);
    if(badge)badge.style.display=state.unreadChat>0?'flex':'none';
  }
  function addChipHud(){
    const stats=document.querySelector('.header-stats');if(!stats||document.getElementById('chips-balance'))return;
    const el=document.createElement('div');el.id='chips-balance';el.className='balance chips-balance';
    el.innerHTML=chipIcon()+'<span id="chips-display">'+fmt(state.chips)+'</span><span class="currency-name">CHIPS</span>';
    stats.insertBefore(el,stats.firstChild);updateHeaderFeature();
  }
  setTimeout(addChipHud,0);

  // Dynamic rival instances: database IDs remain stable for history, names do not.
  function rivalInstance(base,boss=false){
    const b=boss?bossify(base):{...base};
    return {...b,name:boss?b.name:dynName(b),status:boss?'BOSS':rand(BOT_STATUS),instanceId:String(base.id)+'_'+Date.now()+'_'+Math.random().toString(36).slice(2,7)};
  }

  const baseRenderOpponents=renderOpponents;
  renderOpponents=function(){
    if(state.duelSub==='tour'){
      // Tournament grid is always visible; locked nodes show their condition.
      const root=document.getElementById('opponent-list');if(!root)return;
      const car=carsDB.find((x:any)=>x.id===state.activeCarId);if(!car){baseRenderOpponents();return;}
      const myPower=getEffectivePower(car);
      root.innerHTML='<div class="tournament-grid">'+tournamentsDB.map((o:any,i:number)=>{
        const locked=state.level<Number(o.unlockLevel||1);
        const prev=i>0?tournamentsDB[i-1]:null;
        const prevRuns=prev?Number((state.tournamentRuns[String(prev.id)]||{}).count||0):0;
        const prevOk=!prev||prevRuns>0;
        const ratingOk=Number(state.districtRep||0)>=i*250;
        const open=!locked&&prevOk&&ratingOk;
        const boss= i===tournamentsDB.length-1;
        const display=boss?dynName(o,0,true):dynName(o);
        const reason=locked?'Нужен LVL '+o.unlockLevel:!prevOk?'Победи предыдущего босса':!ratingOk?'Нужен рейтинг '+(i*250):'ДОСТУПЕН';
        return '<article class="tournament-node '+(open?'open ':'locked ')+(boss?'boss':'')+'"><div class="tn-kicker">'+(boss?'BOSS · ':'')+'ЭТАП '+(i+1)+'</div><h3>'+escapeHtml(display)+'</h3><p>'+escapeHtml(o.name)+' · '+o.power+' л.с.</p><div class="tn-reward">'+fmt(o.reward)+' SYND</div><div class="tn-lock">'+(open?'✓ ОТКРЫТО':'🔒 '+escapeHtml(reason))+'</div><button class="btn '+(open?'btn-gold':'btn-ghost')+'" '+(open?'':'disabled')+' onclick="prepareRace(\''+String(o.id)+'\',\'tour\')">'+(open?'ВЫЙТИ НА ЛИНИЮ':'ЗАБЛОКИРОВАНО')+'</button></article>';
      }).join('')+'</div>';
      return;
    }
    const root=document.getElementById('opponent-list');
    if(!root){baseRenderOpponents();return;}
    // street chaos entry is rendered above normal grid
    baseRenderOpponents();
    setTimeout(()=>{
  ...
  function startSpecialChase(){
    ...
    if(c){
      c.mode='chase';c.lanes=2;c.chase=true;c.isPolice=true;c.trackLength=1500;c.distance=80;c.aiDistance=0;
      c.chipsReward=2+Math.floor(Math.random()*4);c.syndReward=Math.round(economyRaceReward({power:getEffectivePower(car),unlockLevel:state.level})*1.75);c.radarChance=0;
  ...
  (window as any).startHardcoreRace=startHardcoreRace;

  // Hardcore shifting: progressively narrower, moving green zone and −40% speed on red.
  const baseManualShift=manualShift;
  manualShift=function(){
    const c=raceCtx;if(!c||c.finished)return;
    if(!c.hardcore){baseManualShift();return;}
    if(c.gear>=6){baseManualShift();return;}
    const p=c.rpm/c.redline, progress=Math.min(1,c.shiftCount/6);
    const green=Math.max(.028,.105-progress*.012);
    const center=Math.max(.58,Math.min(.84,.70+(Math.random()-.5)*.12));
    const good=Math.abs(p-center)<=green*2.15;
    const perfect=Math.abs(p-center)<=green;
    c.shiftCount++;c.gear=Math.min(6,c.gear+1);c.shiftCenter=center;c.greenWidth=green;
    if(perfect){
      c.perfectShifts++;c.goodShifts++;c.rpm=Math.max(3200,c.rpm*.94);haptic('success');showShiftText('HARDCORE · ИДЕАЛЬНО',true);
    }else if(good){
      c.goodShifts++;c.rpm=Math.max(2700,c.rpm*.90);haptic('medium');showShiftText('HARDCORE · ХОРОШО',false);
    }else{
      c.errors++;c.overheat=true;c.rpm=Math.max(1800,c.rpm*.55);c.speed*=.60; // transient drop; simulation maintains reduced cap below
      c.overheatTimer=2.4;haptic('warning');showShiftText('ПЕРЕГРЕВ · −40% СКОРОСТИ',false);
    }
    updateRaceHUD();
  };
  const baseSimulate=simulateRace;
  simulateRace=function(dt){
    const c=raceCtx;
    if(c?.hardcore&&c.overheatTimer>0){
      c.overheatTimer=Math.max(0,c.overheatTimer-dt);
      const r=baseSimulate(dt);
      if(c) c.speed=Math.min(c.speed,c.maxSpeed*.60);
      return r;
    }
  ...
  finishRace=function(playerWins,c){
    ...
    if(ctx?.chase){
      ...
      if(playerWins){
        const reward=Math.max(2, Number(ctx.chipsReward)||2);
  ...
  const baseUpgrade=upgradeTune;
  upgradeTune=function(carId,key){
    const car=carsDB.find((x:any)=>x.id===carId),upg=getUpg(carId),lvl=Number(upg[key]||0);
    if(!car||lvl>=5){baseUpgrade(carId,key);return;}
    const chipCost=1+Math.floor(lvl/2);
    if(!chipSpend(chipCost,'tuning_'+key)){openTune(carId);return;}
    const before=lvl;
    baseUpgrade(carId,key);
    if(Number(getUpg(carId)[key]||0)<=before){state.chips+=chipCost;showToast('Установка не выполнена — Чипы возвращены');}
  ...
  function generateRealPlate(country='RU',region?:string){
    ...
    let num='',letters='';
    if(premium){
      const p=rand(['777','001','111','555','999']);
      num=p;letters=rand(['AAA','XXX','BBB','CCC']);
    }else{
      num=String(Math.floor(Math.random()*1000)).padStart(3,'0');
      letters=rand(PLATE_LETTERS)+rand(PLATE_LETTERS)+rand(PLATE_LETTERS);
    }
  ...
  (window as any).showMarketVehicleDetail=function(raw:any){
    const v=raw||{};const root=document.getElementById('money-modal-root');if(!root)return;
    const d=v.vehicle_data||v.vehicle||v, upg=d.upgrades||{};
    root.innerHTML='<div class="modal-overlay"><div class="vehicle-detail-modal"><div class="section-title"><span>'+escapeHtml(d.name||'Автомобиль')+'</span></div><div class="plate-preview">'+escapeHtml(d.plate||'—')+'</div><div class="vehicle-detail-grid"><span>Пробег <b>'+fmt(Number(d.mileage)||0)+' км</b></span><span>Мощность <b>'+fmt(Number(d.power)||0)+' л.с.</b></span><span>Двигатель <b>Ур. '+(upg.engine||0)+'</b></span><span>Турбо <b>Ур. '+(upg.turbo||0)+'</b></span><span>КПП <b>Ур. '+(upg.transmission||0)+'</b></span><span>Сцепление <b>Ур. '+(upg.grip||0)+'</b></span></div><p>'+escapeHtml(d.description||d.flavor||'Описание отсутствует.')+'</p><button class="btn btn-select" onclick="closeMoneyModal()">ЗАКРЫТЬ</button></div></div>';
  };
  // Maintenance: oil, tires, engine wear affects power and can be serviced.
  function maintenanceFor(id){
    if(!state.maintenance[id])state.maintenance[id]={oil:100,tires:100,engine:100};
    return state.maintenance[id];
  }
  function servicePart(id,part){
    const car=carsDB.find((x:any)=>x.id===id);if(!car)return;
    const m=maintenanceFor(id),price=Math.max(50,Math.round(car.price*.025));
    if(state.coins<price){showToast('Недостаточно SYND на ТО');return;}
  ...
  // Passive wear each race.
  const baseFinish2=finishRace;
  finishRace=function(playerWins,c){
    const car=carsDB.find((x:any)=>x.id===state.activeCarId),m=car?maintenanceFor(car.id):null;
    if(m){m.oil=Math.max(0,m.oil-(c?.hardcore?4:2));m.tires=Math.max(0,m.tires-(c?.chaos?3:1));m.engine=Math.max(0,m.engine-(c?.errors>1?4:2));}
    baseFinish2(playerWins,c);
  };

  // Garage / dealer carousel.
  function plateVisual(){
    const p=state.plates?.[state.activePlate];
    if(!p)return '<div class="plate-visual empty">NO PLATE</div>';
  ...
  function plateVisualForCar(carId:any){
    try{const uid=state.installedPlates?.[String(Number(carId))],p=state.plateInventory?.find((x:any)=>x.uid===uid);if(p?.text)return '<div class="plate-visual"><span class="plate-main">'+escapeHtml(p.text)+'</span></div>';}catch(_){}
  ...
  setTimeout(renderChatBadge,250);
  const oldOpenChat=openChat;
  openChat=function(){state.unreadChat=0;updateHeaderFeature();oldOpenChat();};
  /* v15: removed fake random chat unread ticker; unread changes only from real messages. */
  ...
  const featureUpdateHUD=updateRaceHUD;
  updateRaceHUD=function(){
    featureUpdateHUD();
    const c=raceCtx;if(!c?.hardcore)return;
    const dial=document.getElementById('rpm-dial');if(dial){
      const center=c.shiftCenter??.70,green=c.greenWidth??.10;
      const centerDeg=center*264-132;
      dial.style.setProperty('--green-start',(centerDeg-green*132)+'deg');
      dial.style.setProperty('--green-end',(centerDeg+green*132)+'deg');
      dial.classList.toggle('hardcore-vibrate',!!c.overheatTimer);
    }
    const status=document.getElementById('shift-help');
    if(status)status.textContent='HARDCORE · зелёная зона '+Math.round((c.greenWidth??.10)*100)+'% · зона смещается';
  };

  const featureOpenTune=openTune;
  openTune=function(carId:any){
    featureOpenTune(carId);
    setTimeout(()=>{
      document.querySelectorAll('#tune-list .tune-btn').forEach((btn:any)=>{
        const onclick=String(btn.getAttribute('onclick')||''),m=onclick.match(/upgradeTune\((\d+),'([^']+)'/);
        if(!m)return;
        const lvl=Number(getUpg(Number(m[1]))[m[2]]||0),chips=1+Math.floor(lvl/2);
        if(lvl<5)btn.innerHTML=btn.textContent+' · '+chips+' ◈';
      });
      const title=document.getElementById('tune-car-title');if(title&&!title.textContent?.includes('CHIPS'))title.textContent+=' · SYND + CHIPS';
    },0);
  };

  // Improve market listings without changing the server contract.
  const oldRenderMarketList=renderMarketList;
  renderMarketList=function(rows:any[]){
    oldRenderMarketList(rows);
    setTimeout(()=>{
      document.querySelectorAll('#market-list [data-listing-id], #market-list .market-card').forEach((el:any)=>{
        if(!el.querySelector?.('.market-detail-link'))return;
      });
    },0);
  };
  ...
  (window as any).openTune=openTune;
  (window as any).upgradeTune=upgradeTune;
  (window as any).prepareRace=prepareRace;
...
/* ==================== UX + GAMEPLAY FIXPACK 14 ==================== */
(() => {
  const WANTED_DECAY_MS=30*60*1000;
  const wantedCost=75000;
  const priorHeaderV14=updateHeader;
  updateHeader=function(){priorHeaderV14();const chips=document.getElementById('chips-display');if(chips)chips.textContent=fmt(Number(state.chips)||0);};
  (window as any).updateHeader=updateHeader;
  function syncWantedDecayV14(){
    const now=Date.now();
    if(!Number.isFinite(Number(state.heatUpdatedAt))||state.heatUpdatedAt<=0)state.heatUpdatedAt=now;
    if(state.heat>0){
      const steps=Math.floor((now-state.heatUpdatedAt)/WANTED_DECAY_MS);
      if(steps>0){state.heat=Math.max(0,state.heat-steps);state.heatUpdatedAt+=steps*WANTED_DECAY_MS;saveState();}
    }else state.heatUpdatedAt=now;
  }
  function wantedMultV14(level=state.heat){return [1,1,1.5,2,3,5][Math.max(0,Math.min(5,Number(level)||0))]||1;}
  function wantedEffectsV14(level=state.heat){
    return [
      'Розыска нет.',
      'Пока без дополнительных эффектов.',
      'Штрафы ×1.5.',
      'Штрафы ×2.0 · 30% шанс патрульного штрафа перед дуэлью.',
      'Штрафы ×3.0 · 50% шанс подставного NPC в списке дуэлей.',
      'Штрафы ×5.0 · возможна ОБЛАВА с уменьшением призового фонда вдвое.'
    ][Math.max(0,Math.min(5,Number(level)||0))];
  }
  function afterWantedChangeV14(){updateHeader();saveState();if(document.getElementById('screen-profile')?.classList.contains('active'))renderProfile();try{void syncPlayerProfile(true);}catch(_){}}
  addHeat=function(n=1){syncWantedDecayV14();state.heat=Math.max(0,Math.min(5,(Number(state.heat)||0)+Math.max(0,Number(n)||0)));state.heatUpdatedAt=Date.now();afterWantedChangeV14();};
  reduceHeat=function(n=1){syncWantedDecayV14();state.heat=Math.max(0,(Number(state.heat)||0)-Math.max(0,Number(n)||0));state.heatUpdatedAt=Date.now();afterWantedChangeV14();};
  renderHeatStrip=function(id){syncWantedDecayV14();const el=document.getElementById(id);if(!el)return;el.classList.add('heat-clickable-v14');el.setAttribute('role','button');el.setAttribute('tabindex','0');el.setAttribute('onclick','openWantedInfoV14()');el.innerHTML='<div class="heat-head"><span>РОЗЫСК · '+heatLabel()+'</span><b>'+state.heat+' / 5 ⭐</b></div><div class="heat-bars">'+[1,2,3,4,5].map(i=>'<i class="'+(i<=state.heat?'on':'')+'"></i>').join('')+'</div><small class="heat-hint-v14">Нажми, чтобы посмотреть эффекты</small>';};
  (window as any).openWantedInfoV14=function(){syncWantedDecayV14();const root=document.getElementById('police-modal-root');if(!root)return;const next=state.heat>0?Math.max(0,WANTED_DECAY_MS-(Date.now()-state.heatUpdatedAt)):0;const mins=Math.ceil(next/60000);root.innerHTML='<div class="modal-overlay" onclick="if(event.target===this)closePoliceModal()"><div class="police-modal wanted-modal-v14"><div class="police-title">РОЗЫСК '+state.heat+' ⭐</div><div class="wanted-stars-v14">'+[1,2,3,4,5].map(i=>'<span class="'+(i<=state.heat?'on':'')+'">★</span>').join('')+'</div><div class="police-line">'+escapeHtml(wantedEffectsV14())+'</div>'+(state.heat?'<div class="wanted-timer-v14">Следующая звезда исчезнет примерно через '+mins+' мин.</div><button class="big-btn" '+(state.coins<wantedCost?'disabled':'')+' onclick="buyWantedReductionV14()">СНИЗИТЬ РОЗЫСК · −'+fmt(wantedCost)+' SYND</button>':'<div class="result-reward" style="color:var(--green)">ТЫ ЧИСТ</div>')+'<button class="btn btn-ghost" style="margin-top:8px" onclick="closePoliceModal()">ЗАКРЫТЬ</button></div></div>';};
  (window as any).buyWantedReductionV14=function(){syncWantedDecayV14();if(state.heat<=0)return;if(state.coins<wantedCost){showToast('Не хватает SYND');return;}state.coins-=wantedCost;state.stats.totalSpent+=wantedCost;reduceHeat(1);updateHeader();saveState();renderProfile();(window as any).openWantedInfoV14();};

  // Garage carousel: center focus, snap, mouse wheel/keyboard/touch and state synchronization.
  function bindGarageCarouselV14(){
    const root=document.getElementById('garage-list');if(!root||root.dataset.v14Bound==='1')return;root.dataset.v14Bound='1';root.tabIndex=0;
    let touchX=0,touchY=0,horizontal=false,raf=0;
    const focusCenter=()=>{raf=0;const cards=[...root.querySelectorAll('.carousel-card')];if(!cards.length)return;const rr=root.getBoundingClientRect(),center=rr.left+rr.width/2;let best=null,dist=Infinity;cards.forEach((card:any)=>{const r=card.getBoundingClientRect(),d=Math.abs((r.left+r.width/2)-center);if(d<dist){dist=d;best=card;}});cards.forEach((card:any)=>card.classList.toggle('is-centered-v14',card===best));};
  ...
  function refreshCarDependentUiV14(){
    updateHeader();try{(window as any).updateHeaderFeature?.();}catch(_){}
    if(document.getElementById('opponent-list'))renderOpponents();
    if(document.getElementById('screen-profile')?.classList.contains('active'))renderProfile();
  }
  selectCar=function(carId){
    carId=Number(carId);if(!state.ownedCars.includes(carId))return;
    state.activeCarId=carId;saveState();refreshCarDependentUiV14();renderGarage();showToast('Машина выбрана · АКТИВНА ✓');haptic('success');
  };

  const priorGarageV14=renderGarage;
  renderGarage=function(){priorGarageV14();const root=document.getElementById('garage-list');if(root){root.querySelectorAll('.carousel-card').forEach((card:any)=>{const id=Number(card.dataset.carId);card.classList.toggle('is-active',id===state.activeCarId);const btn=card.querySelector('.btn-select');if(btn&&id===state.activeCarId){btn.textContent='ВЫБРАНА ✓';btn.disabled=true;}else if(btn){btn.textContent='ВЫБРАТЬ';btn.disabled=false;}});bindGarageCarouselV14();requestAnimationFrame(()=>{const active=root.querySelector('.carousel-card[data-car-id="'+state.activeCarId+'"]') as HTMLElement|null;if(active&&document.getElementById('screen-garage')?.classList.contains('active')) active.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});});}refreshCarDependentUiV14();};

  // Premium showroom is intentionally a grid, not a second garage carousel.
  renderShop=function(){
    updateHeader();renderShopToolbar();const root=document.getElementById('shop-list');if(!root)return;root.className='list-container showroom-grid-v14';
    let list=carsDB.filter((c:any)=>shopCategory==='all'||c.cat===shopCategory);list=shopSort==='power'?list.slice().sort((a:any,b:any)=>b.power-a.power):list.slice().sort((a:any,b:any)=>a.price-b.price);
    root.innerHTML=list.map((car:any)=>{const owned=state.ownedCars.includes(car.id),missing=Math.max(0,car.price-state.coins),vmax=Math.round(Math.min(430,165+Number(car.power)*.20)),zero=Math.max(1.9,Math.min(10.5,9.8-Math.log2(Math.max(120,car.power)/120)*1.45));return '<article class="showroom-card-v14 cat-'+escapeHtml(String(car.cat||'street'))+'"><div class="showroom-media-v14" onclick="openDetail('+car.id+')">'+carArtSVG(car)+'<div class="showroom-class-v14">'+escapeHtml(CAT_LABELS[car.cat]||car.tier||'STREET')+'</div></div><div class="showroom-body-v14"><div class="showroom-title-v14"><b>'+escapeHtml(car.name)+'</b><span>'+escapeHtml(car.tier||'')+'</span></div><div class="showroom-stats-v14"><span><small>МОЩНОСТЬ</small><b>'+fmt(car.power)+' л.с.</b></span><span><small>0–100</small><b>≈ '+zero.toFixed(1)+' c</b></span><span><small>МАКС.</small><b>≈ '+vmax+' км/ч</b></span></div><div class="showroom-price-v14">'+(car.price?fmt(car.price)+' SYND':'СТАРТОВАЯ')+'</div>'+(owned?'<button class="btn btn-select" onclick="switchTab(\'garage\')">В ГАРАЖ</button>':'<button class="btn btn-buy" '+(missing?'disabled':'')+' onclick="buyCar('+car.id+')">'+(missing?'НЕ ХВАТАЕТ '+fmt(missing)+' SYND':'КУПИТЬ')+'</button>')+'</div></article>';}).join('');
  };

  // Wanted events, arrest gate and chase preparation.
  const priorPrepareV14=prepareRace;
  prepareRace=function(target:any,mode:any){
    syncWantedDecayV14();
    if(Number(state.arrestUntil)>Date.now()){showToast('Временный арест · дуэли доступны через '+Math.ceil((state.arrestUntil-Date.now())/1000)+' сек.');switchTab('garage');return;}
    if((mode==='normal'||mode==='tour'||!mode)&&state.heat>=3&&Math.random()<.30){const fine=Math.round(50*wantedMultV14());state.coins=Math.max(0,state.coins-fine);state.stats.finesPaid+=fine;state.stats.finesCount++;showToast('Патруль засёк тебя · −'+fmt(fine)+' SYND');updateHeader();saveState();}
    priorPrepareV14(target,mode);const c=raceCtx;if(!c)return;
    if(c.chase||c.isPolice||c.opp?.chase||c.opp?.isPolice){c.chase=true;c.isPolice=true;c.trackLength=1500;c.distance=80;c.aiDistance=0;c.aiFinishedAt=0;c.chasePrevGap=80;}
  ...
  renderOpponents=function(){priorOppV14();syncWantedDecayV14();const root=document.getElementById('opponent-list');if(!root)return;if(state.heat>=4&&state.duelSub!=='tour'&&Math.random()<.50){const cards=[...root.querySelectorAll('.duel-card-v127')];if(cards.length){const card=cards[Math.floor(Math.random()*cards.length)] as HTMLElement;const btn=card.querySelector('button[onclick*="prepareRace"]') as HTMLButtonElement|null;if(btn&&!btn.disabled){btn.dataset.original=btn.getAttribute('onclick')||'';btn.setAttribute('onclick','undercoverDuelV14()');card.dataset.undercover='1';}}}
    const summary=document.getElementById('duel-match-summary'),car=carsDB.find((x:any)=>x.id===state.activeCarId);if(summary&&car&&!summary.querySelector('.active-plate-duel-v14'))summary.insertAdjacentHTML('beforeend','<div class="active-plate-duel-v14">'+plateBadgeV14(car.id)+'</div>');
  };
  (window as any).undercoverDuelV14=function(){const fine=Math.round(POLICE_BASE_FINE*wantedMultV14()),seize=Math.max(0,Math.round(state.coins*(.05+Math.random()*.08)));state.coins=Math.max(0,state.coins-fine-seize);state.stats.finesPaid+=fine;state.stats.finesCount++;addHeat(1);updateHeader();saveState();const root=document.getElementById('police-modal-root');if(root)root.innerHTML='<div class="modal-overlay"><div class="police-modal"><div class="police-title">🚔 ПОДСТАВНОЙ ЗАЕЗД</div><div class="police-line">Соперник оказался сотрудником ДПС.</div><div class="result-reward">ШТРАФ: −'+fmt(fine)+' SYND</div>'+(seize?'<div class="result-reward">КОНФИСКАЦИЯ: −'+fmt(seize)+' SYND</div>':'')+'<button class="big-btn" onclick="closePoliceModal();renderOpponents()">ПОНЯТНО</button></div></div>';};

  function plateBadgeV14(carId:any){let text='';try{const uid=state.installedPlates?.[String(Number(carId))],p=state.plateInventory?.find((x:any)=>x.uid===uid);text=p?.text||'';}catch(_){}return '<span class="plate-badge-v14 '+(text?'':'empty')+'">'+escapeHtml(text||'БЕЗ НОМЕРА')+'</span>'; }
  const priorProfileV14=renderProfile;
  renderProfile=function(){priorProfileV14();const heat=document.getElementById('profile-heat'),car=carsDB.find((x:any)=>x.id===state.activeCarId);if(heat&&car){let card=document.getElementById('profile-active-car-v14');if(!card){card=document.createElement('div');card.id='profile-active-car-v14';card.className='profile-active-car-v14';heat.insertAdjacentElement('afterend',card);}card.innerHTML='<div class="profile-active-media-v14">'+carArtSVG(car)+'</div><div><small>АКТИВНАЯ МАШИНА</small><b>'+escapeHtml(car.name)+'</b>'+plateBadgeV14(car.id)+'</div>';}};

  // Chase-specific HUD and finish screen; normal zero rewards become meaningful text.
  const priorHudV14=updateRaceHUD;
  updateRaceHUD=function(){priorHudV14();const c=raceCtx;if(!c)return;const gap=(c.distance||0)-(c.aiDistance||0),gapEl=document.getElementById('race-gap'),leadEl=document.getElementById('race-lead'),cine=document.getElementById('cine-gap-label'),map=document.querySelector('.race-map.race-cinematic');if(c.chase||c.isPolice){map?.classList.add('chase-v14');const prev=Number.isFinite(Number(c.chasePrevGap))?Number(c.chasePrevGap):gap,closing=prev>gap+.35;c.chasePrevGap=gap;const label=closing&&gap<45?'ДПС ДОГОНЯЕТ!':'ДПС +'+Math.max(0,Math.round(gap))+' м ПОЗАДИ';if(gapEl){gapEl.textContent=label;gapEl.classList.toggle('chase-closing-v14',closing);}if(leadEl)leadEl.textContent=label+' · до выхода '+Math.max(0,Math.ceil(c.trackLength-c.distance))+' м';if(cine){cine.textContent=label;cine.classList.toggle('chase-closing-v14',closing);}}else{map?.classList.remove('chase-v14');const prev=Number.isFinite(Number(c.gapPrevV14))?Number(c.gapPrevV14):gap,delta=gap-prev;c.gapPrevV14=gap;let stateText='РОВНО';if(gap<-2&&delta>.12)stateText='ДОГОНЯЮ';else if(gap<-2&&delta>.02)stateText='РЫВКОМ';else if(delta<-.12)stateText='ОТРЫВАЮТСЯ';else if(gap>4)stateText='ВПЕРЕДИ';if(leadEl){leadEl.dataset.gapState=stateText;leadEl.classList.toggle('catching-v14',stateText==='ДОГОНЯЮ');leadEl.classList.toggle('surge-v14',stateText==='РЫВКОМ');leadEl.classList.toggle('losing-v14',stateText==='ОТРЫВАЮТСЯ');}}};

  const priorFinishV14=finishRace;
  finishRace=function(playerWins:any,cArg:any){
    const c=cArg||raceCtx;if(!c)return;
    if(c.chase||c.isPolice||c.opp?.chase||c.opp?.isPolice){
      if(c.finished)return;c.finished=true;try{if(c.raf)cancelAnimationFrame(c.raf);}catch(_){}try{if(c.launchIv)clearInterval(c.launchIv);}catch(_){}
      const car=carsDB.find((x:any)=>x.id===state.activeCarId);state.stats.races++;if(car)state.condition[car.id]=Math.max(0,getCondition(car.id)-3);
      const root=document.getElementById('race-content');
      if(playerWins){
        state.stats.wins++;state.winStreak=(state.winStreak||0)+1;const synd=Math.max(120,Number(c.syndReward)||Math.round(economyRaceReward({power:getEffectivePower(car),unlockLevel:state.level})*1.75)),chips=Math.max(2,Number(c.chipsReward)||2);state.coins+=synd;state.stats.totalEarned+=synd;state.chips=(Number(state.chips)||0)+chips;state.chipLedger=(state.chipLedger||[]).concat({delta:chips,reason:'special_chase_escape',ts:Date.now()}).slice(-100);reduceHeat(1);addXP(24);recordRaceTelemetry({ts:Date.now(),won:true,opponent:'ДПС',route:c.route||'Спец-погоня',time:c.elapsed,topSpeed:c.topSpeed||c.speed,perfectShifts:c.perfectShifts||0,nitroUsed:c.nitroUsed});root.innerHTML='<div class="race3"><div class="result-box win chase-result-v14"><div class="result-title">ПОБЕГ УДАЛСЯ</div><div class="result-sub">Ты дотянул до выхода из зоны погони. ДПС не успел сравняться.</div><div class="result-reward">+'+fmt(synd)+' SYND</div><div class="result-reward chips-v14">+'+fmt(chips)+' ◈ ЧИПОВ</div><div class="xp-gain-box">⭐ +24 XP · '+Number(c.elapsed||0).toFixed(2)+' c · МАКС. '+Math.round(c.topSpeed||c.speed||0)+' км/ч</div></div><div class="list-container"><button class="btn btn-select" onclick="switchTab(\'duel-select\')">ЕЩЁ ЗАЕЗД</button><button class="btn btn-ghost" onclick="switchTab(\'garage\')">В ГАРАЖ</button></div></div>';
      }else{
        state.stats.losses++;state.winStreak=0;addHeat(1);const fine=Math.max(120,Math.round(POLICE_BASE_FINE*wantedMultV14())),eventCash=Math.random()<.55?Math.max(0,Math.round(state.coins*(.05+Math.random()*.10))):0;state.coins=Math.max(0,state.coins-fine-eventCash);state.stats.finesPaid+=fine;state.stats.finesCount++;let eventText='';if(eventCash){eventText='<div class="result-reward">КОНФИСКАЦИЯ: −'+fmt(eventCash)+' SYND</div>';}else{state.arrestUntil=Date.now()+60000;eventText='<div class="arrest-v14">ВРЕМЕННЫЙ АРЕСТ · 60 СЕКУНД · дуэли недоступны</div>';}addXP(4);recordRaceTelemetry({ts:Date.now(),won:false,opponent:'ДПС',route:c.route||'Спец-погоня',time:c.elapsed,topSpeed:c.topSpeed||c.speed,perfectShifts:c.perfectShifts||0,nitroUsed:c.nitroUsed});root.innerHTML='<div class="race3"><div class="result-box lose chase-result-v14"><div class="result-title">🚔 ТЕБЯ ДОГНАЛИ</div><div class="result-sub">ПОПРОБУЙ УЙТИ ОТ ПОГОНИ</div><div class="result-reward">ШТРАФ: −'+fmt(fine)+' SYND</div>'+eventText+'<div class="xp-gain-box">⭐ +4 XP · награды за поражение нет</div></div><div class="list-container"><button class="btn btn-select" onclick="switchTab(\'garage\')">В ГАРАЖ</button><button class="btn btn-ghost" onclick="switchTab(\'jobs\')">ПОДРАБОТКА</button></div></div>';
      }
      updateHeader();try{(window as any).updateHeaderFeature?.();}catch(_){}saveState();checkAchievements();return;
    }
    priorFinishV14(playerWins,cArg);const reward=document.querySelector('#race-content .result-reward');if(reward&&/^\+?0\s*SYND/i.test(String(reward.textContent||'').trim()))reward.textContent='БЕЗ НАГРАДЫ';
  };

  // Wanted-scaled police choices.
  policeChoice=function(choice:any){syncWantedDecayV14();const mult=wantedMultV14(),root=document.getElementById('police-modal-root');let html='';if(choice==='pay'){const fine=Math.round(POLICE_BASE_FINE*mult);state.coins=Math.max(0,state.coins-fine);state.stats.finesPaid+=fine;state.stats.finesCount++;reduceHeat(1);html='<div class="police-line">Штраф оплачен.</div><div class="result-reward">−'+fmt(fine)+' SYND</div>';}else if(choice==='negotiate'){if(Math.random()<.48){const bribe=Math.round(POLICE_BASE_FINE*.5*mult);state.coins=Math.max(0,state.coins-bribe);html='<div class="police-line">Договорились.</div><div class="result-reward">−'+fmt(bribe)+' SYND</div>';}else{const fine=Math.round(POLICE_BASE_FINE*1.5*mult);state.coins=Math.max(0,state.coins-fine);state.stats.finesPaid+=fine;state.stats.finesCount++;addHeat(1);html='<div class="police-line">Не вышло. Розыск вырос.</div><div class="result-reward">−'+fmt(fine)+' SYND</div>';}}else{addHeat(1);if(Math.random()<.35)html='<div class="police-line">Ты сорвался с места и ушёл. Розыск вырос.</div><div class="result-reward" style="color:var(--green)">УШЁЛ БЕЗ ШТРАФА</div>';else{const fine=Math.round(POLICE_BASE_FINE*2*mult);state.coins=Math.max(0,state.coins-fine);state.stats.finesPaid+=fine;state.stats.finesCount++;html='<div class="police-line">Попытка спорить закончилась штрафом.</div><div class="result-reward">−'+fmt(fine)+' SYND</div>';}}updateHeader();saveState();if(root)root.innerHTML='<div class="modal-overlay"><div class="police-modal"><div class="police-title">РЕШЕНИЕ ДПС</div>'+html+'<button class="big-btn" onclick="closePoliceModal();switchTab(\'profile\')">ПОНЯТНО</button></div></div>';};

  // Final image-fit and lane arrangement hooks happen in CSS; render once with v14 logic.
  (window as any).selectCar=selectCar;(window as any).renderGarage=renderGarage;(window as any).renderShop=renderShop;(window as any).renderOpponents=renderOpponents;(window as any).renderProfile=renderProfile;(window as any).prepareRace=prepareRace;(window as any).finishRace=finishRace;(window as any).updateRaceHUD=updateRaceHUD;(window as any).policeChoice=policeChoice;
  syncWantedDecayV14();renderGarage();renderShop();
})();

/* ==================== ECONOMY + WORK + UX UPDATE 15 ==================== */
(() => {
  const CHIP_QUEST_TARGET=3;
  const TOW_TRUCK_PRICE=650000;
  const TOW_SHIFT_MS=15*60*1000;
  const DELIVERY_COOLDOWN_MS=30*60*1000;
  const BOT_VISUAL_NAMES=['Шахист_77','StreetKing','Дядя Вася','TurboRacer','Smokie_Rider','NightFox','Black_ICE','Redline','GhostDriver','Механик_13','Kira','Rook','Vanta','Lynx','Nocturne'];

  function ensureWorkV15(){
    if(!plainObject(state.work)) state.work={};
    const w=state.work;
    w.taxiJobs=Math.max(0,Number(w.taxiJobs)||0);w.theftJobs=Math.max(0,Number(w.theftJobs)||0);
    w.towTruckOwned=w.towTruckOwned===true;w.towTruckUntil=Math.max(0,Number(w.towTruckUntil)||0);w.towTruckStartedAt=Math.max(0,Number(w.towTruckStartedAt)||0);w.towTruckReward=Math.max(0,Number(w.towTruckReward)||0);
    w.activeType=String(w.activeType||'');w.activeUntil=Math.max(0,Number(w.activeUntil)||0);w.activeStartedAt=Math.max(0,Number(w.activeStartedAt)||0);w.activeReward=Math.max(0,Number(w.activeReward)||0);w.activeCarId=Math.max(0,Number(w.activeCarId)||0);
    const day=new Date().toISOString().slice(0,10);
    if(w.chipQuestDay!==day){w.chipQuestDay=day;w.chipQuestProgress=0;w.chipQuestClaimed=false;}
    w.chipQuestProgress=Math.max(0,Math.min(CHIP_QUEST_TARGET,Number(w.chipQuestProgress)||0));w.deliveryReadyAt=Math.max(0,Number(w.deliveryReadyAt)||0);
    return w;
  }
  ensureWorkV15();

  function addChipsV15(n:number,reason:string){
    n=Math.max(0,Math.floor(Number(n)||0));if(!n)return false;
    state.chips=Math.max(0,Number(state.chips)||0)+n;
    if(!Array.isArray(state.chipLedger))state.chipLedger=[];
    state.chipLedger.push({delta:n,reason,ts:Date.now()});state.chipLedger=state.chipLedger.slice(-100);
    updateHeader();try{(window as any).updateHeaderFeature?.();}catch(_){}saveState();return true;
  }
  function spendChipsV15(n:number,reason:string){
    n=Math.max(0,Math.floor(Number(n)||0));
    if((Number(state.chips)||0)<n){showToast('Недостаточно Чипов');return false;}
    state.chips=Math.max(0,(Number(state.chips)||0)-n);
    if(!Array.isArray(state.chipLedger))state.chipLedger=[];
    state.chipLedger.push({delta:-n,reason,ts:Date.now()});state.chipLedger=state.chipLedger.slice(-100);
    updateHeader();try{(window as any).updateHeaderFeature?.();}catch(_){}saveState();return true;
  }
  (window as any).addChipsV15=addChipsV15;
  ...
  function settleWorkV15(){
    const w=ensureWorkV15(),now=Date.now();
    if(!w.activeType||!w.activeUntil||now<w.activeUntil)return false;
    const type=w.activeType,reward=Math.max(0,Number(w.activeReward)||0),car=carsDB.find((c:any)=>c.id===Number(w.activeCarId))||workCarV15();
    clearActiveWorkV15();
    if(type==='taxi'){
      w.taxiJobs++;recordContractEvent('job',1);moneyAddV15(reward,'ТАКСИ · ЗАКАЗ ВЫПОЛНЕН');addXP(7);
    }else if(type==='theft'){
      w.theftJobs++;recordContractEvent('job',1);
      const caught=Math.random()<Math.min(.42,.30+(Number(state.heat)||0)*.025);
      if(caught){const fine=Math.min(state.coins,Math.max(12000,Math.round(reward*.35)));state.coins=Math.max(0,state.coins-fine);state.stats.finesPaid+=fine;state.stats.finesCount++;addHeat(1);showToast('ПЕРЕГОН · ДПС ПЕРЕХВАТИЛ · −'+fmt(fine)+' SYND');haptic('warning');}
      else{moneyAddV15(reward,'ПЕРЕГОН · МАШИНА ДОСТАВЛЕНА');addXP(11);}
    }else if(type==='delivery'){
      addChipsV15(1,'parts_delivery');chipQuestProgressV15();w.deliveryReadyAt=Date.now()+DELIVERY_COOLDOWN_MS;showToast('ДЕТАЛИ ДОСТАВЛЕНЫ · +1 ЧИП');addXP(5);
    }
    updateHeader();saveState();checkAchievements();return true;
  }

  function startWorkV15(type:string){
    const w=ensureWorkV15();settleWorkV15();if(w.activeType){showToast('Сначала закончи текущую подработку');return;}
    const car=workCarV15();if(!car)return;
    const now=Date.now();let seconds=6,reward=0;
    if(type==='taxi'){seconds=taxiDurationV15(car);reward=taxiRewardV15(car);}
    else if(type==='theft'){seconds=Math.max(6,taxiDurationV15(car)+2);reward=theftRewardV15(car);}
    else if(type==='delivery'){
      if(now<Number(w.deliveryReadyAt||0)){showToast('Доставка деталей снова будет доступна позже');return;}
      seconds=8;reward=0;
    }else return;
    w.activeType=type;w.activeStartedAt=now;w.activeUntil=now+seconds*1000;w.activeReward=reward;w.activeCarId=car.id;
    saveState();if(type==='delivery')renderChipCenterV15();else renderJobs();haptic('medium');
  }
  ...
  function buyChipItemV15(item:string){
    const car=workCarV15();if(!car)return;
    const prices:any={nitro:2,service:2,heat:3,license:4};const cost=prices[item];if(!spendChipsV15(cost,'chip_shop_'+item))return;
    if(item==='nitro')state.nitro=(Number(state.nitro)||0)+2;
    if(item==='service'){state.fuel[car.id]=100;state.condition[car.id]=100;if(state.maintenance?.[car.id])state.maintenance[car.id]={oil:100,tires:100,engine:100};}
    if(item==='heat')state.heat=Math.max(0,(Number(state.heat)||0)-1);
    if(item==='license'){state.licenseSuspended=false;state.hasLicense=true;}
    updateHeader();saveState();renderChipCenterV15();showToast('ПОКУПКА ЗА ЧИПЫ ВЫПОЛНЕНА');
  }

  function ensureChipCenterV15(){
    const main=document.getElementById('main-scroll');if(!main)return null;let screen=document.getElementById('screen-chips');
    if(!screen){screen=document.createElement('div');screen.id='screen-chips';screen.className='screen';screen.innerHTML='<div class="back-link" onclick="switchTab(\'more\')">← Меню</div><div class="section-title"><span>Чип-контракты</span></div><div id="chip-center-v15"></div>';main.appendChild(screen);}
    return screen;
  }
  function renderChipCenterV15(){
    settleWorkV15();ensureChipCenterV15();const root=document.getElementById('chip-center-v15');if(!root)return;const w=ensureWorkV15(),now=Date.now(),deliveryRemain=Math.max(0,Number(w.deliveryReadyAt||0)-now),active=w.activeType&&w.activeUntil>now;
    root.innerHTML='<div class="chip-center-hero-v15"><span>ПРЕМИАЛЬНАЯ ЭКОНОМИКА</span><h2>'+fmt(state.chips)+' ◈ ЧИПОВ</h2><p>Чипы не участвуют в казино и обычной подработке. Их дают редкие контракты, боссы и специальные погони.</p></div>'+
      '<article class="work-card-v15 chips"><div class="work-icon-v15">◈</div><div><h3>Доставка редких деталей</h3><p>Отдельный премиальный контракт. Выполни маршрут — получи Чип без ставки SYND.</p><div class="work-meta-v15"><span>Награда <b>+1 ЧИП</b></span><span>'+(deliveryRemain?'Кулдаун <b>'+Math.ceil(deliveryRemain/60000)+' мин.</b>':'Доступно <b>СЕЙЧАС</b>')+'</span></div></div><button '+(active||deliveryRemain?'disabled':'')+' onclick="startWorkV15(\'delivery\')">ДОСТАВИТЬ</button></article>'+
      '<section class="chip-contract-v15"><div><span>ДНЕВНОЙ ЧИП-КОНТРАКТ</span><h3>Закрой '+CHIP_QUEST_TARGET+' редкие доставки</h3><p>Это отдельная премиальная цепочка и она не меняет выплату обычной подработки.</p></div><div class="chip-contract-progress-v15"><i style="width:'+Math.round(w.chipQuestProgress/CHIP_QUEST_TARGET*100)+'%"></i></div><b>'+w.chipQuestProgress+' / '+CHIP_QUEST_TARGET+'</b><button '+(w.chipQuestClaimed||w.chipQuestProgress<CHIP_QUEST_TARGET?'disabled':'')+' onclick="claimChipQuestV15()">'+(w.chipQuestClaimed?'ПОЛУЧЕНО':'ЗАБРАТЬ 2 ЧИПА')+'</button></section>'+
      '<section class="chip-shop-v15"><div class="v8-section-head"><b>МАГАЗИН ЗА ЧИПЫ</b><span>'+fmt(state.chips)+' ◈</span></div><div class="chip-shop-grid-v15"><button onclick="buyChipItemV15(\'nitro\')"><b>2 ◈</b><span>+2 нитро</span></button><button onclick="buyChipItemV15(\'service\')"><b>2 ◈</b><span>Полное ТО</span></button><button onclick="buyChipItemV15(\'heat\')"><b>3 ◈</b><span>−1 розыск</span></button><button onclick="buyChipItemV15(\'license\')"><b>4 ◈</b><span>Вернуть права</span></button></div></section>';
  }

  renderJobs=function(){
    settleWorkV15();updateHeader();const root=document.getElementById('jobs-list');if(!root)return;const w=ensureWorkV15(),car=workCarV15(),now=Date.now();
    const active=w.activeType&&w.activeUntil>now;const total=Math.max(1,w.activeUntil-w.activeStartedAt),left=Math.max(0,w.activeUntil-now),progress=active?Math.max(0,Math.min(100,100-left/total*100)):0;
    const activeLabel=w.activeType==='taxi'?'ТАКСИ':w.activeType==='theft'?'ПЕРЕГОН':w.activeType==='delivery'?'ПРЕМИАЛЬНЫЙ КОНТРАКТ':'';
    const activeHtml=active?'<section class="work-active-v15"><div class="work-active-head"><span>'+activeLabel+'</span><b>'+Math.ceil(left/1000)+' СЕК.</b></div><div class="work-road-v15"><div class="work-route-v15"></div><div class="work-car-v15" style="left:calc('+progress+'% - 24px)"><img src="'+escapeHtml(carThumb(car))+'" alt=""></div></div><div class="work-progress-v15"><i style="width:'+progress+'%"></i></div><small>Маршрут выполняется в реальном времени. Можно уйти со страницы и вернуться позже.</small></section>':'';
    const towRemaining=Math.max(0,w.towTruckUntil-now),towReady=w.towTruckUntil>0&&towRemaining<=0;
  ...
  doJob=function(jobId:any){startWorkV15(jobId==='theft'?'theft':'taxi');};

  (window as any).startWorkV15=startWorkV15;(window as any).buyTowTruckV15=buyTowTruckV15;(window as any).startTowShiftV15=startTowShiftV15;(window as any).collectTowShiftV15=collectTowShiftV15;(window as any).claimChipQuestV15=claimChipQuestV15;(window as any).buyChipItemV15=buyChipItemV15;(window as any).renderJobs=renderJobs;(window as any).renderChipCenterV15=renderChipCenterV15;(window as any).doJob=doJob;

  // Casino is back on Syndicate Coins. Chips stay premium and are never wagered.
  const baseClampV15=clampBet;
  clampBet=function(input:any,min:any){return baseClampV15(input,Math.max(1000,Number(min)||0));};
  (window as any).clampBet=clampBet;
  const casinoBase=renderCasinoHub;
  renderCasinoHub=function(){casinoBase();const hero=document.querySelector('#screen-casino .casino-hero-premium p');if(hero)hero.textContent='Ставки только в Синдикат коинах. Чипы — премиальная валюта и здесь не используются.';};
  (window as any).renderCasinoHub=renderCasinoHub;

  // Horizontal modern dealership.
  renderShop=function(){
    updateHeader();renderShopToolbar();const root=document.getElementById('shop-list');if(!root)return;root.className='list-container showroom-horizontal-v15';
    let list=carsDB.filter((c:any)=>shopCategory==='all'||c.cat===shopCategory);list=shopSort==='power'?list.slice().sort((a:any,b:any)=>b.power-a.power):list.slice().sort((a:any,b:any)=>a.price-b.price);
    root.innerHTML=list.map((car:any)=>{const owned=state.ownedCars.includes(car.id),need=Math.max(0,car.price-state.coins);return '<article class="dealer-card-v15 '+(owned?'owned':'')+'"><div class="dealer-media-v15" onclick="openDetail('+car.id+')">'+carArtSVG(car)+'<span>'+escapeHtml(String(CAT_LABELS[car.cat]||car.tier||'AUTO'))+'</span></div><div class="dealer-copy-v15"><div><small>'+escapeHtml(String(car.tier||''))+'</small><h3>'+escapeHtml(car.name)+'</h3></div><div class="dealer-specs-v15"><span><small>МОЩНОСТЬ</small><b>'+fmt(car.power)+' л.с.</b></span><span><small>0–100</small><b>~'+Math.max(2.2,(9.8-car.power/190)).toFixed(1)+' c</b></span><span><small>КЛАСС</small><b>'+escapeHtml(String(CAT_LABELS[car.cat]||car.cat))+'</b></span></div><div class="dealer-price-v15">'+fmt(car.price)+' <small>SYND</small></div>'+(owned?'<button onclick="switchTab(\'garage\')">В ГАРАЖЕ · ОТКРЫТЬ</button>':'<button '+(need?'disabled':'')+' onclick="buyCar('+car.id+')">'+(need?'НЕ ХВАТАЕТ '+fmt(need):'КУПИТЬ')+'</button>')+'</div></article>';}).join('');
  };
  (window as any).renderShop=renderShop;
  ...
  renderHeatStrip=function(id:any){const el=document.getElementById(id);if(!el)return;el.classList.add('heat-clickable-v14','wanted-v15');el.setAttribute('role','button');el.setAttribute('tabindex','0');el.setAttribute('onclick','openWantedInfoV14()');el.innerHTML='<div class="heat-head"><span class="wanted-title-v15"><svg class="wanted-icon-v15" viewBox="0 0 32 32" aria-hidden="true"><path d="M16 3 20 8l6 1-2 6 3 5-5 4-2 6-4-3-4 3-2-6-5-4 3-5-2-6 6-1 4-5Z"/><circle cx="16" cy="15" r="5"/><path d="M13 15h6M16 12v6"/></svg><span>РОЗЫСК · '+heatLabel()+'</span></span><b>'+state.heat+' / 5</b></div><div class="heat-bars">'+[1,2,3,4,5].map(i=>'<i class="'+(i<=state.heat?'on':'')+'"></i>').join('')+'</div><small class="heat-hint-v14">Нажми, чтобы посмотреть эффекты и способы снизить розыск</small>';};
  (window as any).renderHeatStrip=renderHeatStrip;

  // Remove fake unread notifications. Only real chat loading can change unread counters now.
  state.unreadChat=0;updateHeader();saveState();

  // Opponent variety is installed by the consistency guard below so list and race share one generated instance.

  // Race visibility hardening: inject real vehicle images into cinematic cars and keep all lanes inside the road viewport.
  const cockpitBaseV15=showRaceCockpit;
  showRaceCockpit=function(){cockpitBaseV15();const c=raceCtx;if(!c)return;const me=workCarV15(),playerBody=document.querySelector('#cine-player .cine-car-body');if(playerBody)playerBody.innerHTML='<img class="cine-vehicle-img-v15" src="'+escapeHtml(carThumb(me))+'" alt="'+escapeHtml(me.name)+'">';const rivals=[...document.querySelectorAll('.cine-car[data-rival]')];rivals.forEach((el:any,i:number)=>{let rc:any;if(c.chase||c.isPolice)rc=carsDB.find((x:any)=>x.id===10)||carsDB[0];else if(c.chaosBots?.[i])rc=rivalCatalogCar(c.chaosBots[i]);else rc=rivalCatalogCar(c.opp);const body=el.querySelector('.cine-car-body');if(body&&rc)body.innerHTML='<img class="cine-vehicle-img-v15" src="'+escapeHtml(carThumb(rc))+'" alt="'+escapeHtml(rc.name)+'">';});};
  (window as any).showRaceCockpit=showRaceCockpit;

  // Cases: stale local opening flag must never strand the screen. Server-pending reconcile remains authoritative.
  if(state.caseOpening){state.caseOpening=false;saveState();setTimeout(()=>{try{void (window as any).reconcileCaseRollsV15?.();renderCases();}catch(_){}},120);}

  // Premium chips: boss wins are a rare source, not a casino bankroll.
  const finishBaseV15=finishRace;
  finishRace=function(playerWins:any,cArg:any){const c=cArg||raceCtx,bossWin=!!playerWins&&!!c?.opp?.boss&&!c?.chase&&!c?.isPolice;finishBaseV15(playerWins,cArg);if(bossWin){(window as any).addChipsV15?.(1,'boss_win');showToast('БОСС ПОВЕРЖЕН · +1 ЧИП');}};
  (window as any).finishRace=finishRace;
...
(() => {
  const BOT_VISUAL_NAMES_V15_GLOBAL=['Шахист_77','StreetKing','Дядя Вася','TurboRacer','Smokie_Rider','NightFox','Black_ICE','Redline','GhostDriver','Механик_13','Kira','Rook','Vanta','Lynx','Nocturne'];
  // Case opening watchdog: UI must recover in-place even if network/animation callbacks never return.
  const openCaseBaseV15=openCase;
  openCase=async function(caseId:any){
    const started=Date.now();
    const watchdog=setTimeout(()=>{
      if(state.caseOpening){
        state.caseOpening=false;saveState();
        try{renderCases();}catch(_){}
        try{void (window as any).reconcileCaseRollsV15?.();}catch(_){}
        showToast('Кейс восстановлен без перезапуска. Проверь награду в истории.');
      }
    },12000);
    try{return await openCaseBaseV15(caseId);}finally{
      // Keep the watchdog alive while the visual reel is still animating; otherwise clear it.
      if(!state.caseOpening||Date.now()-started>11000)clearTimeout(watchdog);
    }
  };
  (window as any).openCase=openCase;

  // Keep the opponent shown in the list consistent with the one entering the race.
  const renderOppBaseV15b=renderOpponents;
  renderOpponents=function(){
    renderOppBaseV15b();
    if(state.duelSub==='tour')return;
    const usedNames=new Set<string>(),usedCars=new Set<number>();
    document.querySelectorAll('#opponent-list .duel-card-v127').forEach((card:any,idx:number)=>{
      const btn=card.querySelector('button[onclick*="prepareRace"]');if(!btn)return;
      const m=String(btn.getAttribute('onclick')||'').match(/prepareRace\('([^']+)'/);if(!m)return;
      const opp=opponentsDB.find((o:any)=>String(o.id)===String(m[1]));if(!opp)return;
      let name=BOT_VISUAL_NAMES_V15_GLOBAL[(idx+Math.floor(Math.random()*BOT_VISUAL_NAMES_V15_GLOBAL.length))%BOT_VISUAL_NAMES_V15_GLOBAL.length];
      let g=0;while(usedNames.has(name)&&g++<20)name=BOT_VISUAL_NAMES_V15_GLOBAL[Math.floor(Math.random()*BOT_VISUAL_NAMES_V15_GLOBAL.length)];usedNames.add(name);
      const candidates=realCarCatalog().slice().sort((a:any,b:any)=>Math.abs(Number(a.power)-Number(opp.power))-Math.abs(Number(b.power)-Number(opp.power))).slice(0,6).filter((c:any)=>!usedCars.has(c.id));
      const vc=(candidates.length?candidates:realCarCatalog())[Math.floor(Math.random()*Math.max(1,(candidates.length?candidates:realCarCatalog()).length))];
      opp.v15DisplayName=name;if(vc){opp.v15VisualCarId=vc.id;usedCars.add(vc.id);}
      const nameEl=card.querySelector('.duel-name-v127 b');if(nameEl)nameEl.textContent=name;
      if(vc){const img=card.querySelector('.duel-car-v127 img');if(img){img.src=carThumb(vc);img.alt=vc.name;}const sub=card.querySelector('.duel-core-v127 > small');if(sub)sub.textContent=vc.name;}
    });
  };
  (window as any).renderOpponents=renderOpponents;

  const prepareBaseV15b=prepareRace;
  prepareRace=function(target:any,mode:any){
    const source=(mode==='tour'?tournamentsDB:opponentsDB).find((o:any)=>String(o.id)===String(target));
    prepareBaseV15b(target,mode);
    if(raceCtx&&source&&mode!=='tour'&&!raceCtx.chase&&!raceCtx.isPolice){
      if(source.v15DisplayName)raceCtx.opp.name=source.v15DisplayName;
      if(source.v15VisualCarId)raceCtx.opp.v15VisualCarId=source.v15VisualCarId;
    }
  };
  (window as any).prepareRace=prepareRace;

  const cockpitBaseV15b=showRaceCockpit;
  showRaceCockpit=function(){
    cockpitBaseV15b();const c=raceCtx;if(!c)return;
    document.querySelectorAll('.cine-car[data-rival]').forEach((el:any,i:number)=>{
      let rc:any=null;if(c.opp?.v15VisualCarId&&i===0)rc=carsDB.find((x:any)=>x.id===Number(c.opp.v15VisualCarId));
      if(!rc&&c.chaosBots?.[i]?.v15VisualCarId)rc=carsDB.find((x:any)=>x.id===Number(c.chaosBots[i].v15VisualCarId));
      if(!rc)return;const img=el.querySelector('.cine-vehicle-img-v15');if(img){img.src=carThumb(rc);img.alt=rc.name;}
    });
  };
  (window as any).showRaceCockpit=showRaceCockpit;
})();
...
/* ==================== RECOVERY 17.1 · SAFE TUNING BRIDGE ====================
   Core garage/race/duels intentionally stay on the proven V16 renderer.
   The Pixi atelier is progressive enhancement only: if WebGL fails, gameplay remains intact. */
(() => {
  function visualConfigForCarV171(carId:any){
    const id=Math.max(1,Math.trunc(Number(carId)||Number(state.activeCarId)||1));
    try{return normalizeCarVisualConfig(state.carVisuals?.[String(id)]||defaultCarVisualConfig(id),id);}catch(_){return defaultCarVisualConfig(id);}
  }
  function setVisualConfigForCarV171(carId:any,raw:any){
    const id=Math.max(1,Math.trunc(Number(carId)||Number(state.activeCarId)||1));
    if(!plainObject(state.carVisuals))state.carVisuals={};
    try{state.carVisuals[String(id)]=normalizeCarVisualConfig(raw,id);}catch(_){state.carVisuals[String(id)]=defaultCarVisualConfig(id);}
    return state.carVisuals[String(id)];
  }
  function openVisualAtelierV171(carId:any){
    const id=Math.max(1,Math.trunc(Number(carId)||Number(state.activeCarId)||1));
    const car=carsDB.find((x:any)=>Number(x.id)===id);
    if(!car||!state.ownedCars.includes(id)){showToast('Сначала добавь машину в гараж');return;}
    window.dispatchEvent(new CustomEvent('autosyndicate:open-atelier',{detail:{carId:id,carName:car.name,config:visualConfigForCarV171(id)}}));
  }
  function injectAtelierButtonsV171(){
    document.querySelectorAll('#garage-list .garage-card-v151,#garage-list .carousel-card').forEach((card:any)=>{
      if(card.querySelector('.atelier-launch-v171'))return;
      const id=Number(card.dataset.carId)||0;if(!id||!state.ownedCars.includes(id))return;
      const actions=card.querySelector('.garage-actions-v151')||card.querySelector('.carousel-actions')||card;
      const b=document.createElement('button');b.className='btn btn-gold atelier-launch-v171';b.type='button';b.textContent='ТЮНИНГ-АТЕЛЬЕ';
      b.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();openVisualAtelierV171(id);};actions.appendChild(b);
    });
  }
  const garageBaseV171=renderGarage;
  renderGarage=function(){const r=garageBaseV171();requestAnimationFrame(injectAtelierButtonsV171);return r;};
  (window as any).renderGarage=renderGarage;
  (window as any).openVisualAtelierV171=openVisualAtelierV171;
  (window as any).openVisualAtelierV17=openVisualAtelierV171;
  (window as any).__AUTOSYNDICATE_GET_CAR_VISUAL__=(carId:any)=>visualConfigForCarV171(carId);

  window.addEventListener('autosyndicate:visual-save',(event:any)=>{
    const id=Math.max(1,Math.trunc(Number(event?.detail?.carId)||0));
    if(!id||!state.ownedCars.includes(id))return;
    setVisualConfigForCarV171(id,event?.detail?.config);
  ...
  requestAnimationFrame(injectAtelierButtonsV171);
})();


/* ==================== RECOVERY 17.2 · PIXI RACE + LAYERED VISUAL PIPELINE ==================== */
(() => {
  let raceVisualEngineV172:any=null;
  let raceVisualTokenV172=0;

  function carVisualV172(carId:any){
    const id=Math.max(1,Math.trunc(Number(carId)||Number(state.activeCarId)||1));
    try{return normalizeCarVisualConfig(state.carVisuals?.[String(id)]||defaultCarVisualConfig(id),id);}catch(_){return defaultCarVisualConfig(id);}
  }

  // Restore one visual contract across garage, shop, profile and legacy cards.
  carThumb=function(car:any){
    const id=Math.max(1,Math.trunc(Number(car?.id)||1));
    return carVisualDataUri(carVisualV172(id));
  };
  carArtSVG=function(car:any){
    const id=Math.max(1,Math.trunc(Number(car?.id)||1));
    return carVisualSvgMarkup(carVisualV172(id),{className:'car-vector-art-v172',ariaLabel:String(car?.name||'Автомобиль')});
  };

  function destroyRaceVisualV172(){
    raceVisualTokenV172++;
    if(raceVisualEngineV172){try{raceVisualEngineV172.destroy();}catch(_){}raceVisualEngineV172=null;}
  }

  function npcVisualV172(carId:any,seed=0){
    const id=Math.max(1,Math.trunc(Number(carId)||1));
    const base:any=defaultCarVisualConfig(id);
    const palette=['#f8fafc','#ef4444','#2563eb','#16a34a','#f59e0b','#7c3aed','#111827'];
    return normalizeCarVisualConfig({...base,paint:{...base.paint,hex:palette[Math.abs(Number(seed)||id)%palette.length]}},id);
  }

  function privateVisualV172(c:any,remote:boolean){
    try{
      const payload=c?.privateDuelPayloadV16||c?.privatePayload||null;if(!payload)return null;
      const role=payload?.role,room=payload?.room||{};
      ...
      const profile=(payload?.profiles||[]).find((x:any)=>x?.id===playerId);
      return normalizeCarVisualConfig(profile?.car_visuals?.[String(carId)],carId);
    }catch(_){return null;}
  }

  const privateStartBaseV172=(window as any).__AUTOSYNDICATE_START_PRIVATE_DUEL__;
  if(typeof privateStartBaseV172==='function'){
    (window as any).__AUTOSYNDICATE_START_PRIVATE_DUEL__=(payload:any)=>{
      privateStartBaseV172(payload);
      if(raceCtx){raceCtx.privateDuelPayloadV16=payload;raceCtx.privateLocalVisualV172=privateVisualV172(raceCtx,false);raceCtx.privateRemoteVisualV172=privateVisualV172(raceCtx,true);}
    };
  }

  function raceRacersV172(c:any){
    const rows:any[]=[];
    if(c.chase||c.isPolice){
      const copId=10,base:any=defaultCarVisualConfig(copId);
      rows.push({id:'rival-0',label:'ДПС',kind:'cop',visual:normalizeCarVisualConfig({...base,paint:{...base.paint,hex:'#f8fafc'},tint:{...base.tint,opacity:.9}},copId)});
    }else if(c.chaosBots?.length){
      c.chaosBots.forEach((bot:any,i:number)=>{const carId=Number(bot?.v15VisualCarId)||Number(rivalCatalogCar(bot)?.id)||1;rows.push({id:'rival-'+i,label:bot?.name||('BOT '+(i+1)),kind:'rival',visual:npcVisualV172(carId,i+carId)});});
    }else{
      const rc=rivalCatalogCar(c.opp||{}),carId=Number(c.opp?.v15VisualCarId)||Number(rc?.id)||1;
      rows.push({id:'rival-0',label:c.opp?.name||'RIVAL',kind:'rival',visual:c.privateRemoteVisualV172||npcVisualV172(carId,Number(c.opp?.power)||carId)});
    }
    rows.push({id:'player',label:state.playerName||'ВЫ',kind:'player',visual:c.privateLocalVisualV172||carVisualV172(state.activeCarId)});
    return rows;
  }

  function raceDistancesV172(c:any){
    const rows:any[]=[];
    if(c.chase||c.isPolice)rows.push({id:'rival-0',distance:Number(c.aiDistance)||0});
    else if(c.chaosBots?.length){const n=c.chaosBots.length;c.chaosBots.forEach((_:any,i:number)=>{const factor=1+(i-(n-1)/2)*.032;rows.push({id:'rival-'+i,distance:Math.max(0,(Number(c.aiDistance)||0)*factor)});});}
    else {const remote=Number(c.privateRemoteProgress?.distance);rows.push({id:'rival-0',distance:Number.isFinite(remote)?remote:(Number(c.aiDistance)||0)});}
    rows.push({id:'player',distance:Number(c.distance)||0});
    return rows;
  }

  function mountRaceVisualV172(c:any){
    destroyRaceVisualV172();
    const host=document.getElementById('pixi-race-host-v172') as HTMLElement|null;if(!host)return;
    const token=++raceVisualTokenV172;
    void import('@/features/race/pixi/PixiDragRaceEngine').then(async(mod)=>{
      if(token!==raceVisualTokenV172||!document.body.contains(host))return;
      try{
        const engine=await mod.mountPixiDragRace(host,{racers:raceRacersV172(c),reducedMotion:!!state.settings?.reducedMotion});
        if(token!==raceVisualTokenV172||!document.body.contains(host)){engine.destroy();return;}
        raceVisualEngineV172=engine;
        engine.update({playerDistance:Number(c.distance)||0,speedKmh:Number(c.speed)||0,racers:raceDistancesV172(c),trackLength:Math.max(1,Number(c.trackLength)||402)});
      }catch(error){
        // No text fallback: keep an actual canvas surface even if both Pixi renderers fail unexpectedly.
        console.error('[Race] visual renderer failed',error);
        const canvas=document.createElement('canvas');canvas.className='pixi-race-canvas-v172';canvas.width=Math.max(320,host.clientWidth||320);canvas.height=Math.max(220,host.clientHeight||260);host.replaceChildren(canvas);
        const ctx=canvas.getContext('2d');if(ctx){ctx.fillStyle='#101827';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#27292d';ctx.fillRect(0,canvas.height*.48,canvas.width,canvas.height*.52);ctx.strokeStyle='rgba(255,255,255,.25)';ctx.beginPath();ctx.moveTo(0,canvas.height*.62);ctx.lineTo(canvas.width,canvas.height*.62);ctx.moveTo(0,canvas.height*.82);ctx.lineTo(canvas.width,canvas.height*.82);ctx.stroke();}
      }
    });
  }

  const cockpitBaseV172=showRaceCockpit;
  showRaceCockpit=function(){
    cockpitBaseV172();const c:any=raceCtx;if(!c)return;
    const map=document.querySelector('.race-map.race-cinematic') as HTMLElement|null;if(!map)return;
    map.classList.add('pixi-race-map-v172');
    map.innerHTML='<div id="pixi-race-host-v172" class="pixi-race-host-v172" aria-label="2.5D трасса"></div>'+ 
      '<div class="pixi-race-overlay-v172"><span id="pixi-lane-label-v172">'+(c.chase||c.isPolice?'ПОГОНЯ':'DRAG · 2.5D')+'</span><b id="pixi-speed-label-v172">0 КМ/Ч</b></div>'+ 
      '<div class="cine-hud-bar hud-v16 pixi-hud-v172"><div class="cine-distance-row-v16"><span id="cine-progress-label">ВЫ · 0 м</span><span id="cine-gap-label">СТАРТ</span></div><div class="cine-progress"><i id="cine-progress-fill"></i></div><div class="chase-meter-v16" id="chase-meter-v16"><i id="chase-meter-fill-v16"></i></div></div>';
    const up=document.getElementById('shift-btn');
    if(up){const face=up.querySelector('.shift-face');if(face)face.textContent='+';const small=up.querySelector('small');if(small)small.textContent='ПЕРЕДАЧА +';if(!document.getElementById('shift-down-v172')){const down=document.createElement('button');down.id='shift-down-v172';down.className='race-control shift shift-down-v172';down.innerHTML='<span class="shift-face">−</span><small>ПЕРЕДАЧА −</small>';down.onclick=()=>{const gear=Math.max(1,Number(c.gear)||1);if(gear<=1)return;c.gear=gear-1;c.rpm=Math.min(Number(c.redline)||8500,Math.max(1500,speedToRpm(Number(c.speed)||0,c.gear,Number(c.redline)||8500)));updateRaceHUD();};up.before(down);}}
    mountRaceVisualV172(c);
  };
  (window as any).showRaceCockpit=showRaceCockpit;

  const hudBaseV172=updateRaceHUD;
  updateRaceHUD=function(){
    hudBaseV172();const c:any=raceCtx;if(!c)return;
    const speed=document.getElementById('pixi-speed-label-v172');if(speed)speed.textContent=Math.round(Number(c.speed)||0)+' КМ/Ч · '+Math.max(1,Number(c.gear)||1)+' ПЕР.';
    if(raceVisualEngineV172){try{raceVisualEngineV172.update({playerDistance:Number(c.distance)||0,speedKmh:Number(c.speed)||0,racers:raceDistancesV172(c),trackLength:Math.max(1,Number(c.trackLength)||402)});}catch(_){}}
  };
  (window as any).updateRaceHUD=updateRaceHUD;

  const finishBaseV172=finishRace;
  finishRace=function(playerWins:any,cArg:any){destroyRaceVisualV172();return finishBaseV172(playerWins,cArg);};
  (window as any).finishRace=finishRace;

  const switchBaseV172=switchTab;
  switchTab=function(tab:any){if(tab!=='race')destroyRaceVisualV172();const r=switchBaseV172(tab);if(tab==='garage'||tab==='shop'||tab==='profile')requestAnimationFrame(()=>{try{if(tab==='garage')renderGarage();else if(tab==='shop')renderShop();else renderProfile();}catch(_){}});return r;};
  (window as any).switchTab=switchTab;

  // Force current screens to rebuild with the shared visual config instead of stale raster cards.
  try{if(document.getElementById('screen-garage')?.classList.contains('active'))renderGarage();if(document.getElementById('screen-shop')?.classList.contains('active'))renderShop();if(document.getElementById('screen-profile')?.classList.contains('active'))renderProfile();}catch(_){}
})();
