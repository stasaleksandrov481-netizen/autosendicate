...
export async function POST(request:Request){try{assertSameOrigin(request);const session=await requireSession();await enforceRateLimit(session.playerId,'duel-room-write',180,60);const action=duelRoomActionSchema.parse(await request.json());return noStoreJson({ok:true,...await updateDuelRoom(session,action)});}catch(error){return apiError(error,403)}}
