...
export async function GET(){
  ...
  const carRows=(cars.data??[]).filter((c:any)=>Number(c.id)>=1&&Number(c.id)<=25);
  let player=null;
  if(session?.playerId){
    const res=await s.from('player_profiles').select('id,name,level,balance,xp,races,wins,losses,total_earned,owned_cars,active_car_id,rating,best_0_100,profile_tags,car_visuals').eq('id',session.playerId).maybeSingle();
    if(!res.error) player=res.data;
  }
  return noStoreJson({
    ok:true,
    partial:Boolean(cars.error||opponents.error||settings.error),
    cars:carRows.map((c:any)=>({id:c.id,name:c.name,image:null,price:c.price,power:c.power,tier:c.tier,cat:c.category,flavor:c.flavor})),
