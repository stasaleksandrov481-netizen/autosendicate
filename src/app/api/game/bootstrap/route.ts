...
export async function GET(){
  ...
  let player=null;
  if(session?.playerId){
    const res=await s.from('player_profiles').select('id,name,level,balance,xp,races,wins,losses,total_earned,owned_cars,active_car_id,rating,best_0_100,profile_tags').eq('id',session.playerId).maybeSingle();
